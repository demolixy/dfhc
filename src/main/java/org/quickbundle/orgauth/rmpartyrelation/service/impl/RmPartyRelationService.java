...
public class RmPartyRelationService extends RmService implements IRmPartyRelationService, IRmPartyRelationConstants {
    ...
    private void initPartyChildCode(RmPartyRelationVo[] vos) {
    	...
    	//按Party_view_id+Parent_party_code分组处理
        Map<String, List<RmPartyRelationVo>> mpr = new HashMap<String, List<RmPartyRelationVo>>();
        for(int i=0; i<vos.length; i++) {
        	String key = vos[i].getParty_view_id()+"|"+vos[i].getParent_party_code();
        	if(mpr.containsKey(key)) {
        		mpr.get(key).add(vos[i]);
        	} else {
        ...
        //循环mpr Map
        Iterator<String> keyIt = mpr.keySet().iterator();
        String key;
        List<RmPartyRelationVo> lpr = null;
        while(keyIt.hasNext()){
        	key = keyIt.next();
        	lpr = mpr.get(key);
        	//检查key的序列是否存在
        	if(!getSequenceService().isExistsSequence(key)){
        		//插入序列
        		getSequenceService().insert(makeSequenceVo(key));
        	}
        	//循环发号
        	for(RmPartyRelationVo vo:lpr){
        		String nextValue = getSequenceService().doGetNextSequenceValue(key);
        		//父级编码+nextValue转3位16进制
				try {
					vo.setChild_party_code(vo.getParent_party_code()+String.format("%03x",ConvertHelper.toInt(nextValue)));
				} catch (Exception e) {					
					e.printStackTrace();
...
//    	}
     ...
     * @return
     */
	private SequenceVo makeSequenceVo(String key) {
		SequenceVo vo = new SequenceVo();
		vo.setSequenceName(key);
		vo.setCurrentValue(0);
		vo.setCycleType(ISystemConstant.CYCLE_TYPE_NO);
		vo.setRemark(ISystemConstant.RM_PARTY_RELATION_SEQUENCE_REMARK);
		vo.setSequenceLen(4);
		vo.setSequenceRule(ISystemConstant.RM_PARTY_RELATION_SEQUENCE_RULE);
		return vo;
	}  
}