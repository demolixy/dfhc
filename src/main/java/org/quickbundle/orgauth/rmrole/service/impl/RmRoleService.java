...
public class RmRoleService extends RmService implements IRmRoleService, IRmRoleConstants {
    ...
    /**
     * 得到RmPartyRoleService对象
     * 
     * @return Service对象
     */
    public IRmPartyRoleService getRartyRoleService() {
        return (IRmPartyRoleService) RmBeanFactory.getBean(IRmPartyRoleConstants.SERVICE_KEY);  //得到Service对象,受事务控制
    }
    /**
     * 设置数据访问接口
     * 
    ...
    public String insert(RmRoleVo vo) {
    	...
    	//同步插入activiti 组  add by longsebo 2016-11-17
    	activitiService.createGroup(id, RmPartyTypeCache.getSingleton().getPartyType(IRmRoleConstants.TABLE_NAME).getName(), vo.getName());
    ...
    public int delete(String id) {
		...
		//同步删除activiti 组  add by longsebo 2016-11-17
    ...
    public int update(RmRoleVo vo) {
		...
		//同步更新activiti 组  add by longsebo 2016-11-17
    	activitiService.updateGroup(vo.getId(), RmPartyTypeCache.getSingleton().getPartyType(IRmRoleConstants.TABLE_NAME).getName(), vo.getName());
    ...
    public String[] insertRm_party_roles(String id, String[] role_ids) {
		...
		//查询RM_PARTY_ROLE,循环剔除activiti组的用户  add by longsebo 2016-11-17
    	clearOldRoleGroupInActiviti(id);
    	...
    	if(role_ids!=null && role_ids.length >0){
        	...
        	if(role_ids.length > 0) {
            	...
            	for (String role_id1 : role_ids) {
            		...
            		groupIds.add(role_id1);
        		}
            	partyRoleService.insert(prList.toArray(new RmPartyRoleVo[0]));
            	//同步activiti组下的用户  add by longsebo 2016-11-17
            	activitiService.addUseInGroups(id, groupIds);
     ...
     * @param partyId
     */
    private void clearOldRoleGroupInActiviti(String partyId) {
    	String queryCondition="OWNER_PARTY_ID='"+partyId+"'";
		List<RmPartyRoleVo> vos = getRartyRoleService().queryByCondition(queryCondition, null);
		if(vos!=null){
			List<String> groupIds = new ArrayList<String>();
			for(RmPartyRoleVo vo:vos){
				groupIds.add(vo.getRole_id());
			}
			activitiService.removeUseInGroups(partyId, groupIds);
		}
	}
