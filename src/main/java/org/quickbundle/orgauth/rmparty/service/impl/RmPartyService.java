...
public class RmPartyService extends RmService implements IRmPartyService, IRmPartyConstants {
    ...
    /**
     * 得到PartyTypeService对象
     * 
     * @return Service对象
     */
    public IRmPartyTypeService getPartyTypeService() {
        return (IRmPartyTypeService) RmBeanFactory.getBean(IRmPartyTypeConstants.SERVICE_KEY);  //得到Service对象,受事务控制
    }
    /**
     * 设置数据访问接口
     * 
    ...
    public String insert(RmPartyVo vo) {
        ...
        //同步创建activiti group add by longsebo 2016-11-14
        //只有允许挂接用户的party_type的party才有必要创建组
        if(getPartyTypeService().isAllowHangingUser(vo.getParty_type_id())){
        	activitiService.createGroup(id, vo.getParty_type_id_name(),vo.getName());
        }
    ...
    public int delete(String id) {
		...
		//同步删除activiti组  add by longsebo 2016-11-14
    ...
    public int update(RmPartyVo vo) {
    	RmPartyVo oldVo = getDao().find(vo.getId());
    	if(oldVo==null){
    		throw new RuntimeException("根据id:"+vo.getId()+"找不到rm_party记录!");
    	}
    	if(StringHelper.isEmpty(vo.getOld_party_id())){
    		vo.setOld_party_id(oldVo.getOld_party_id());
    	}
    	if(StringHelper.isEmpty(vo.getName())){
    		vo.setName(oldVo.getName());
    	}
    	if(StringHelper.isEmpty(vo.getIs_inherit())){
    		vo.setIs_inherit(oldVo.getIs_inherit());
    	}
    	if(StringHelper.isEmpty(vo.getParty_type_id_name())){
    		vo.setParty_type_id_name(oldVo.getParty_type_id_name());
    	}
    	if(StringHelper.isEmpty(vo.getParty_type_id())){
    		vo.setParty_type_id(oldVo.getParty_type_id());
    	}
    	if(StringHelper.isEmpty(vo.getParty_type_id_name())){
    		vo.setParty_type_id_name(vo.getParty_type_id_name());
    	}
		int sum = getDao().update(vo);
		//同步更新activiti组信息  add by longsebo 2016-11-14
		if(getPartyTypeService().isAllowHangingUser(vo.getParty_type_id())){
			activitiService.updateGroup(vo.getId(), vo.getParty_type_id_name(), vo.getName());
		}
