...
public class RmUserAction extends RmDispatchAction implements IRmUserConstants {
    ...
    /**
     * 得到RmPartyTypeService对象
     * 
     * @return Service对象
     */
    IRmPartyTypeService getRmPartyTypeService() {
    ...
    public ActionForward insert(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        vo.setPassword(RmCryptoHelper.encryptDesBase64(vo.getPassword()));//将用户的密码加密处理
     ...
     * @throws Exception
     */
    public ActionForward gotoinsert(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
    	//查询所有的party_type
    	List<RmPartyTypeVo> vos = getRmPartyTypeService().queryByCondition(null, null);
    	if(vos!=null){
    		String submitBk="";
    		for(int i=0;i<vos.size();i++){
    			submitBk = submitBk+vos.get(i).getBs_keyword();
    			if(i<vos.size()-1){
    				submitBk=submitBk+",";
    			}
    		}
    		request.setAttribute(IRmUserConstants.SUBMIT_BK, submitBk);  //把结果集放入request
    	}else{
    		request.setAttribute(IRmUserConstants.SUBMIT_BK, "");  //把结果集放入request
    	}
