...
public class RmPartyTypeService extends RmService implements IRmPartyTypeService, IRmPartyTypeConstants {
    ...
    public String insert(RmPartyTypeVo vo) {
    	if(StringHelper.isEmpty(vo.getIs_custom_attribute())){
        	vo.setIs_custom_attribute(ISystemConstant.DICTIONARY_RM_YES_NOT_0);
        }
        if(StringHelper.isEmpty(vo.getIs_allow_hanging_user())){
        	vo.setIs_allow_hanging_user(ISystemConstant.DICTIONARY_RM_YES_NOT_0);
        }
        if(StringHelper.isEmpty(vo.getIs_allow_hanging_role())){
        	vo.setIs_allow_hanging_role(ISystemConstant.DICTIONARY_RM_YES_NOT_0);
        }
	...
	@Override
	public boolean isAllowHangingUser(String partyTypeId) {
		RmPartyTypeVo vo = getDao().find(partyTypeId);
		if(vo==null){
			throw new RuntimeException("party type id:"+partyTypeId+"不存在于rm_party_type表!");
		}
		
		return (ISystemConstant.DICTIONARY_RM_YES_NOT_1.equals(vo.getIs_allow_hanging_user()));
	}
