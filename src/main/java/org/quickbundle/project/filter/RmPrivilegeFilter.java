	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws IOException, ServletException {
		...
		try {
			...
			if(RmConfig.getSingleton().isUserDemoMode()) { //demo模式不校验登录
				...
				//TODO 2017 0103 修改
				String pcode=request.getParameter(ISystemConstant.RM_PARENT_CODE);//父节点编码
				pcode=pcode==null?"":pcode;
			
				if(!StringHelper.isEmpty(pcode)){
					session.setAttribute(ISystemConstant.RM_PARENT_CODE, pcode);
				}
				String childSource=request.getParameter(ISystemConstant.RM_CHILD_SOURCE);//子节点编码
				childSource=childSource==null?"":childSource;
				if(!StringHelper.isEmpty(childSource)){
					
					session.setAttribute(ISystemConstant.RM_CHILD_SOURCE, childSource);
				}
