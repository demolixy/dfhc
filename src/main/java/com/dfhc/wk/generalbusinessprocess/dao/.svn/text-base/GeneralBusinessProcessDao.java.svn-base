 ...
 * 系统名称：  qb5&activiti
 *
 * 文件名称：  GeneralBusinessProcessDao.java
 *
 * 功能描述：  通用流程业务表数据访问对象
 * 
 * 版本历史：
 ...
 */
package com.dfhc.wk.generalbusinessprocess.dao;
 ...
 */
@Repository
public class GeneralBusinessProcessDao extends RmSqlSessionDaoSupport implements IGeneralBusinessProcessConstants {
    ...
    public List<GeneralBusinessProcessVo> getByParentId(String parentId){
    	return getSqlSession().selectList(namespace("getByParentId"), parentId);
    }
     ...
     * @return 查询到的VO列表
     */
    public List<GeneralBusinessProcessVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<GeneralBusinessProcessVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           GeneralBusinessProcessVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
