 ...
 *
 * 文件名称：  LadingBillChangeDao.java
 *
 * 功能描述：  司机提货单变车记录数据访问对象
 * 
 * 版本历史：
 * 
 * 2017-01-18   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.ladingbillchange.dao;
 ...
 */
@Repository
public class LadingBillChangeDao extends RmSqlSessionDaoSupport implements ILadingBillChangeConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<LadingBillChangeVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<LadingBillChangeVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           LadingBillChangeVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
