 ...
 *
 * 文件名称：  LadingBillChangeController.java
 *
 * 功能描述：  司机提货单变车记录Controller
 * 
 * 版本历史：
 * 
 * 2017-01-18   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.ladingbillchange.web;
...
import com.dfhc.bus.ladingbillchange.service.LadingBillChangeService;
...
/**
 * list                  /ladingbillchange
 * insert page      GET  /ladingbillchange/insert
 * insert action    POST /ladingbillchange/insert
 * update page      GET  /ladingbillchange/update/{id}
 * update action    POST /ladingbillchange/update
 * delete action    POST /ladingbillchange/delete
 * detail                /ladingbillchange/detail/{id}

 * reference             /ladingbillchange/reference
 * import page      GET  /ladingbillchange/import
 * import action    POST /ladingbillchange/import
 * export custom    GET  /ladingbillchange/export
 * export action    POST /ladingbillchange/export
 */

/**
 * 司机提货单变车记录 Controller
 * 
 * @author 龙色波
 * @see 参见的类
 */
@Controller
@RequestMapping(value = "/ladingbillchange")
public class LadingBillChangeController implements ILadingBillChangeConstants {

    @Autowired
    /**
     * 司机提货单变车记录服务
     */
    private LadingBillChangeService ladingBillChangeService;
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        model.addAttribute("paramLuy", request.getQueryString());
        //单据类型
        model.addAttribute(ISystemConstant.DICTIONARY_BILL_TYPE,RmGlobalReference.get(ISystemConstant.DICTIONARY_BILL_TYPE));
        return "/yulinchemical/bus/ladingbillchange/listLadingBillChange";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        //联查
        ladingBillChangeService.getSearchPara(request,searchPara);
        
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, ladingBillChangeService.getCount(searchPara));
        ...
        List<LadingBillChangeVo> beans = ladingBillChangeService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid LadingBillChangeVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        ladingBillChangeService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid LadingBillChangeVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        ladingBillChangeService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        LadingBillChangeVo bean = ladingBillChangeService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = ladingBillChangeService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += ladingBillChangeService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/ladingbillchange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = ladingBillChangeService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += ladingBillChangeService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/ladingbillchange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/ladingbillchange/detailLadingBillChange";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/ladingbillchange/util/referenceLadingBillChange";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/ladingbillchange/importLadingBillChange";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/ladingbillchange/exportLadingBillChange_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/ladingbillchange/exportLadingBillChange_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("oldLadingBillId", request.getParameter("oldLadingBillId"));
           searchMap.put("oldPlanId", request.getParameter("oldPlanId"));
           searchMap.put("oldOrderPlanCode", request.getParameter("oldOrderPlanCode"));
           searchMap.put("oldBillType", request.getParameter("oldBillType"));
           searchMap.put("newPlanId", request.getParameter("newPlanId"));
           searchMap.put("newOrderPlanCode", request.getParameter("newOrderPlanCode"));
           searchMap.put("newBillType", request.getParameter("newBillType"));
           searchMap.put("modifyApplyPerson", request.getParameter("modifyApplyPerson"));
           searchMap.put("modifyOperator", request.getParameter("modifyOperator"));
           searchMap.put("modifyOperatorId", request.getParameter("modifyOperatorId"));
           searchMap.put("changeTime", request.getParameter("changeTime"));
   ...
   /**
    ...
    public String toDetail(Model model, HttpServletRequest request){    	
    	...
    	LadingBillChangeVo vo=ladingBillChangeService.get(id);
    	
    	model.addAttribute("ladingBillChangeVo", vo);
     ...
     * 变车管理   添加
     */
    @RequestMapping(value = "insertChange", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> insertChange(HttpServletRequest request, @Valid LadingBillChangeVo vo) {
        Map<String, Object> result = new HashMap<String, Object>();
		RmUserVo userVo = (RmUserVo) RmProjectHelper.getRmUserVo(request);
        try {
			
            ladingBillChangeService.insert(request,vo,userVo);  //插入单条记录
            result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
            result.put(ISystemConstant.AJAX_MESSAGE, "新增成功: " + vo.getId());
		} catch (Exception e) {
