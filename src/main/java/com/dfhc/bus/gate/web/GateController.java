...
/**
 * list                  /gate
 * insert page      GET  /gate/insert
 * insert action    POST /gate/insert
 * update page      GET  /gate/update/{id}
 * update action    POST /gate/update
 * delete action    POST /gate/delete
 * detail                /gate/detail/{id}

 * reference             /gate/reference
 * import page      GET  /gate/import
 * import action    POST /gate/import
 * export custom    GET  /gate/export
 * export action    POST /gate/export
 */
...
public class GateController implements IGateConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/gate/listGate";
    }
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid GateVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        gateService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid GateVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        gateService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        GateVo bean = gateService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = gateService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += gateService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/gate?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = gateService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += gateService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/gate?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/gate/detailGate";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/gate/util/referenceGate";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/gate/importGate";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/gate/exportGate_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/gate/exportGate_excel";
    }
