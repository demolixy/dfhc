 ...
 *
 * 文件名称：  ProductInventoryController.java
 *
 * 功能描述：  产品库存中间表Controller
 * 
 * 版本历史：
 * 
 * 2017-03-09   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.productinventory.web;
 ...
 * insert page      GET  /productinventory/insert
 * insert action    POST /productinventory/insert
 * update page      GET  /productinventory/update/{id}
 * update action    POST /productinventory/update
 * delete action    POST /productinventory/delete
 * detail                /productinventory/detail/{id}

 * reference             /productinventory/reference
 * import page      GET  /productinventory/import
 * import action    POST /productinventory/import
 * export custom    GET  /productinventory/export
 * export action    POST /productinventory/export
 */
...
public class ProductInventoryController implements IProductInventoryConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productinventory/listProductInventory";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, productInventoryService.getCount(searchPara));
        ...
        List<ProductInventoryVo> beans = productInventoryService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid ProductInventoryVo vo) {
    	
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        return productInventoryService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid ProductInventoryVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        productInventoryService.update(vo);  //更新单条记录
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        ProductInventoryVo bean = productInventoryService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = productInventoryService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += productInventoryService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/productinventory?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = productInventoryService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += productInventoryService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/productinventory?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productinventory/detailProductInventory";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productinventory/util/referenceProductInventory";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/productinventory/importProductInventory";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/productinventory/exportProductInventory_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/productinventory/exportProductInventory_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("storageLocation", request.getParameter("storageLocation"));
           searchMap.put("storageNumber", request.getParameter("storageNumber"));
           searchMap.put("outboundNumber", request.getParameter("outboundNumber"));
   ...
   /**
    ...
    public String toDetail(Model model, HttpServletRequest request){    	
    	...
    	ProductInventoryVo vo=productInventoryService.get(id);
    	
    	model.addAttribute("productInventoryVo", vo);
