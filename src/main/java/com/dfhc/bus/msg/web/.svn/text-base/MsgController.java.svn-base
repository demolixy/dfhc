...
import com.dfhc.bus.msg.service.MsgService;
...
/**
 * list                  /msg
 * insert page      GET  /msg/insert
 * insert action    POST /msg/insert
 * update page      GET  /msg/update/{id}
 * update action    POST /msg/update
 * delete action    POST /msg/delete
 * detail                /msg/detail/{id}

 * reference             /msg/reference
 * import page      GET  /msg/import
 * import action    POST /msg/import
 * export custom    GET  /msg/export
 * export action    POST /msg/export
 */
...
public class MsgController implements IMsgConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/msg/listMsg";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, msgService.getCount(searchPara));
        ...
        List<MsgVo> beans = msgService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid MsgVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        msgService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid MsgVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        msgService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = msgService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += msgService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/msg?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = msgService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += msgService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/msg?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/msg/detailMsg";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/msg/util/referenceMsg";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/msg/importMsg";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/msg/exportMsg_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/msg/exportMsg_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("senderId", request.getParameter("senderId"));
           searchMap.put("title", request.getParameter("title"));
           searchMap.put("content", request.getParameter("content"));
           searchMap.put("expirationTime", request.getParameter("expirationTime"));
           searchMap.put("messageType", request.getParameter("messageType"));
    ...
    public String doAddMsg(Model model, HttpServletRequest request) {
    	String  idString=RmIdFactory.requestId(TABLE_NAME);
    	model.addAttribute("id", idString);
        return "/yulinchemical/bus/msg/doAddMsg";
    }
    ...
    @RequestMapping(value = "addMsg", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> addMsg(HttpServletRequest request,@Valid MsgVo vo) {
        Map<String, Object> result = new HashMap<String, Object>();
        try {
            msgService.addMsg(request,vo);  //更新单条记录
            result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
            result.put(ISystemConstant.AJAX_MESSAGE, "发送成功");
     		} catch (Exception e) {
    ...
    public String doUpdateMsg(Model model, HttpServletRequest request) {
    	...
    	paraMap.put("messageId", id);
    	List<RecipientsVo> beans=recipientsService.list(paraMap, null);
    	model.addAttribute(ISystemConstant.AJAX_LIST, beans);
    	
    	return "/yulinchemical/bus/msg/toDetailMsg";
    }
