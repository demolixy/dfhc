 ...
 *
 * 文件名称：  OrderRestController.java
 *
 * 功能描述：  销售订单 rest controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.order.rest;
...
import com.dfhc.bus.order.web.OrderController;
 ...
 * insert action        POST    /api/order
 * update action        PUT     /api/order/{id}
 * delete action        DELETE  /api/order/{id}
 * delete multi action  POST    /api/order/delete
 * batch insert update  POST    /api/order/batch
 */
...
public class OrderRestController implements IOrderConstants {
    ...
    public Map<String, Object> list(HttpServletRequest request) {
        Map<String, Object> searchPara = OrderController.getQueryCondition(request);  //从request中获得查询条件
    ...
    public ResponseEntity<?> get(@PathVariable("id") String id) {
        OrderVo task = orderService.get(id);
        if (task == null) {
            return new ResponseEntity<OrderVo>(HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<OrderVo>(task, HttpStatus.OK);
    }
    ...
    @ResponseBody
    public ResponseEntity<?> insert(@RequestBody OrderVo vo, UriComponentsBuilder uriBuilder) {
        //调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
        Set<ConstraintViolation<OrderVo>> failures = validator.validate(vo);
        if (!failures.isEmpty()) {
    ...
    public int delete(HttpServletRequest request) {
        ...
        if (ids != null && ids.length != 0) {
            return orderService.delete(ids); //删除多条记录
        }
        return 0;
    }
    ...
    public ResponseEntity<Map<String, int[]>> batchInsertUpdate(HttpServletRequest request) {
        List<OrderVo> lvo = RmPopulateHelper.populateAjax(OrderVo.class, request);
        for(OrderVo vo : lvo) {
            if(vo.getId() != null) {
        ...
        int[] sum_insert_update = orderService.insertUpdateBatch(lvo.toArray(new OrderVo[0]));
