 ...
 *
 * 文件名称：  CarDistributionPlanDao.java
 *
 * 功能描述：  汽运配送计划数据访问对象
 * 
 * 版本历史：
 ...
 */
package com.dfhc.bus.cardistributionplan.dao;
 ...
 */
@Repository
public class CarDistributionPlanDao extends RmSqlSessionDaoSupport implements ICarDistributionPlanConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<CarDistributionPlanVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<CarDistributionPlanVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           CarDistributionPlanVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
     ...
     * @param vos vo列表
     */
    public void insertBatch1(List<CarDistributionPlanVo> vos) {
    	List<String> idList = null;
    	//统计id为空的数量
    	int requestIdNum = 0;
    	for(int i=0;i<vos.size();i++) {
    	...
    	getSqlSession().update(namespace("insertBatch1"), vos);
    }
}
