 ...
 *
 * 文件名称：  SecurityItemResultController.java
 *
 * 功能描述：  车辆安全检查项目结果Controller
 * 
 * 版本历史：
 * 
 * 2017-04-01   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.securityitemresult.web;
...
/**
 * list                  /securityitemresult
 * insert page      GET  /securityitemresult/insert
 * insert action    POST /securityitemresult/insert
 * update page      GET  /securityitemresult/update/{id}
 * update action    POST /securityitemresult/update
 * delete action    POST /securityitemresult/delete
 * detail                /securityitemresult/detail/{id}

 * reference             /securityitemresult/reference
 * import page      GET  /securityitemresult/import
 * import action    POST /securityitemresult/import
 * export custom    GET  /securityitemresult/export
 * export action    POST /securityitemresult/export
 */

/**
 * 车辆安全检查项目结果 Controller
 * 
 * @author 龙色波
 * @see 参见的类
 */
@Controller
@RequestMapping(value = "/securityitemresult")
public class SecurityItemResultController implements ISecurityItemResultConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/securityitemresult/listSecurityItemResult";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, securityItemResultService.getCount(searchPara));
        ...
        List<SecurityItemResultVo> beans = securityItemResultService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid SecurityItemResultVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        securityItemResultService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid SecurityItemResultVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        securityItemResultService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        SecurityItemResultVo bean = securityItemResultService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = securityItemResultService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += securityItemResultService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/securityitemresult?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = securityItemResultService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += securityItemResultService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/securityitemresult?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/securityitemresult/detailSecurityItemResult";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/securityitemresult/util/referenceSecurityItemResult";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/securityitemresult/importSecurityItemResult";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/securityitemresult/exportSecurityItemResult_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/securityitemresult/exportSecurityItemResult_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("vehicleSafetyInspectionId", request.getParameter("vehicleSafetyInspectionId"));
           searchMap.put("checkType", request.getParameter("checkType"));
           searchMap.put("checkItem", request.getParameter("checkItem"));
           searchMap.put("checkResult", request.getParameter("checkResult"));
           searchMap.put("checkResultCtrlName", request.getParameter("checkResultCtrlName"));
   ...
   /**
    ...
    public String toDetail(Model model, HttpServletRequest request){    	
    	...
    	SecurityItemResultVo vo=securityItemResultService.get(id);
    	
    	model.addAttribute("securityItemResultVo", vo);
