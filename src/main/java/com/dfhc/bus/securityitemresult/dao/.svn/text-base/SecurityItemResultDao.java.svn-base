 ...
 *
 * 文件名称：  SecurityItemResultDao.java
 *
 * 功能描述：  车辆安全检查项目结果数据访问对象
 * 
 * 版本历史：
 * 
 * 2017-04-01   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.securityitemresult.dao;
 ...
 */
@Repository
public class SecurityItemResultDao extends RmSqlSessionDaoSupport implements ISecurityItemResultConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<SecurityItemResultVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<SecurityItemResultVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           SecurityItemResultVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
