 ...
 *
 * 文件名称：  PurchaseIntentionController.java
 *
 * 功能描述：  购买意向管理Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.purchaseintention.web;
...
import com.dfhc.bus.purchaseintention.IPurchaseIntentionConstants;
import com.dfhc.bus.purchaseintention.service.PurchaseIntentionService;
import com.dfhc.bus.purchaseintention.vo.PurchaseIntentionVo;
import com.dfhc.ISystemConstant;
import com.alibaba.fastjson.JSONObject;
import com.dfhc.util.StringHelper;

/**
 * list                  /purchaseintention
 * insert page      GET  /purchaseintention/insert
 * insert action    POST /purchaseintention/insert
 * update page      GET  /purchaseintention/update/{id}
 * update action    POST /purchaseintention/update
 * delete action    POST /purchaseintention/delete
 * detail                /purchaseintention/detail/{id}

 * reference             /purchaseintention/reference
 * import page      GET  /purchaseintention/import
 * import action    POST /purchaseintention/import
 * export custom    GET  /purchaseintention/export
 * export action    POST /purchaseintention/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/purchaseintention")
public class PurchaseIntentionController implements IPurchaseIntentionConstants {

    @Autowired
    /**
     * 购买意向管理服务
     */
    private PurchaseIntentionService purchaseIntentionService;
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/purchaseintention/listPurchaseIntention";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, purchaseIntentionService.getCount(searchPara));
        ...
        List<PurchaseIntentionVo> beans = purchaseIntentionService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid PurchaseIntentionVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        purchaseIntentionService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid PurchaseIntentionVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        purchaseIntentionService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        PurchaseIntentionVo bean = purchaseIntentionService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = purchaseIntentionService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += purchaseIntentionService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/purchaseintention?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = purchaseIntentionService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += purchaseIntentionService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/purchaseintention?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/purchaseintention/detailPurchaseIntention";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/purchaseintention/util/referencePurchaseIntention";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/purchaseintention/importPurchaseIntention";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/purchaseintention/exportPurchaseIntention_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/purchaseintention/exportPurchaseIntention_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("requirement", request.getParameter("requirement"));
           searchMap.put("requirementDate", request.getParameter("requirementDate"));
           searchMap.put("estimatedAmount", request.getParameter("estimatedAmount"));
           searchMap.put("remark", request.getParameter("remark"));
           searchMap.put("noteTaker", request.getParameter("noteTaker"));
           searchMap.put("noteTakerId", request.getParameter("noteTakerId"));
           searchMap.put("recordTime", request.getParameter("recordTime"));
