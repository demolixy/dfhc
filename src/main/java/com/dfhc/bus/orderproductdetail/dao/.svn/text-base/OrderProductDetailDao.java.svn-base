 ...
 *
 * 文件名称：  OrderProductDetailDao.java
 *
 * 功能描述：  销售订单产品明细数据访问对象
 * 
 * 版本历史：
 ...
 */
package com.dfhc.bus.orderproductdetail.dao;
 ...
 */
@Repository
public class OrderProductDetailDao extends RmSqlSessionDaoSupport implements IOrderProductDetailConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<OrderProductDetailVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<OrderProductDetailVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           OrderProductDetailVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
    ...
    /**
     * 查询销售订单明细的总金额和对应客户的可用余额=授信额度+业务可用余额
     * @param params
     * @return
     */
    public Map<String, Object> getCustomerPrice(Map<String, Object> params){
    	return getSqlSession().selectOne(namespace("getCustomerPrice"), params);
    }
}
