 ...
 *
 * 文件名称：  ProductBatchNumberController.java
 *
 * 功能描述：  产品批次记录Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.bus.productbatchnumber.web;
...
/**
 * list                  /productbatchnumber
 * insert page      GET  /productbatchnumber/insert
 * insert action    POST /productbatchnumber/insert
 * update page      GET  /productbatchnumber/update/{id}
 * update action    POST /productbatchnumber/update
 * delete action    POST /productbatchnumber/delete
 * detail                /productbatchnumber/detail/{id}

 * reference             /productbatchnumber/reference
 * import page      GET  /productbatchnumber/import
 * import action    POST /productbatchnumber/import
 * export custom    GET  /productbatchnumber/export
 * export action    POST /productbatchnumber/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/productbatchnumber")
public class ProductBatchNumberController implements IProductBatchNumberConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productbatchnumber/listProductBatchNumber";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, productBatchNumberService.getCount(searchPara));
        ...
        List<ProductBatchNumberVo> beans = productBatchNumberService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid ProductBatchNumberVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        productBatchNumberService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid ProductBatchNumberVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        productBatchNumberService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        ProductBatchNumberVo bean = productBatchNumberService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = productBatchNumberService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += productBatchNumberService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/productbatchnumber?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = productBatchNumberService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += productBatchNumberService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/productbatchnumber?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productbatchnumber/detailProductBatchNumber";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/bus/productbatchnumber/util/referenceProductBatchNumber";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/bus/productbatchnumber/importProductBatchNumber";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/bus/productbatchnumber/exportProductBatchNumber_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/bus/productbatchnumber/exportProductBatchNumber_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("loadingNoticeId", request.getParameter("loadingNoticeId"));
