 ...
 *
 * 文件名称：  ProductBatchNumberDao.java
 *
 * 功能描述：  产品批次记录数据访问对象
 * 
 * 版本历史：
 ...
 */
package com.dfhc.bus.productbatchnumber.dao;
 ...
 */
@Repository
public class ProductBatchNumberDao extends RmSqlSessionDaoSupport implements IProductBatchNumberConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<ProductBatchNumberVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<ProductBatchNumberVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           ProductBatchNumberVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
