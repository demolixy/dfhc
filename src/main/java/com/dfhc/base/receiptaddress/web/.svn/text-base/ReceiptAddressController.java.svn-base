 ...
 *
 * 文件名称：  ReceiptAddressController.java
 *
 * 功能描述：  收货地址维护Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.base.receiptaddress.web;
...
import com.dfhc.base.receiptaddress.service.ReceiptAddressService;
...
/**
 * list                  /receiptaddress
 * insert page      GET  /receiptaddress/insert
 * insert action    POST /receiptaddress/insert
 * update page      GET  /receiptaddress/update/{id}
 * update action    POST /receiptaddress/update
 * delete action    POST /receiptaddress/delete
 * detail                /receiptaddress/detail/{id}

 * reference             /receiptaddress/reference
 * import page      GET  /receiptaddress/import
 * import action    POST /receiptaddress/import
 * export custom    GET  /receiptaddress/export
 * export action    POST /receiptaddress/export
 */
...
public class ReceiptAddressController implements IReceiptAddressConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/receiptaddress/listReceiptAddress";
    }   
    /**
     * ajax 请求列表数据
     * @param model
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, receiptAddressService.getCount(searchPara));
        ...
        List<ReceiptAddressVo> beans = receiptAddressService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid ReceiptAddressVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
//        receiptAddressService.insert(vo);  //插入单条记录
        receiptAddressService.insert(vo,request);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid ReceiptAddressVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        receiptAddressService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        ReceiptAddressVo bean = receiptAddressService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = receiptAddressService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += receiptAddressService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/receiptaddress?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = receiptAddressService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += receiptAddressService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/receiptaddress?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
     ...
     * @return
     */
    @RequestMapping(value="ajaxDeleteReceiptaddress",method=RequestMethod.POST,
    		consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    
    @ResponseBody
    public Map<String,Object> ajaxDeleteReceiptaddress(HttpServletRequest request,Model model){
    	...
    	Map<String,Object> result=new HashMap<String,Object>();
          ...
          int deleteCount = 0;  //定义成功删除的记录数
          if (id != null && id.length() > 0) {
        	  deleteCount = receiptAddressService.deleteLogic(request,id);
          }else{
        	  String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/receiptaddress/detailReceiptAddress";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/receiptaddress/util/referenceReceiptAddress";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/receiptaddress/importReceiptAddress";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/receiptaddress/exportReceiptAddress_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/receiptaddress/exportReceiptAddress_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("recipientPhone", request.getParameter("recipientPhone"));
           searchMap.put("zipcode", request.getParameter("zipcode"));
           searchMap.put("isDefault", request.getParameter("isDefault"));
     ...
     * @return
     */
    @RequestMapping(value="toAddFreight/{id}",method=RequestMethod.GET)
    public String toAddFreight(@PathVariable  String id ,HttpServletRequest request ,Model model){
    	...
    	List<ReceiptAddressVo> listReceiptAddressVo=receiptAddressService.getReceiptCustomer(searchPara, "", 1, -1);
    	
    	if (listReceiptAddressVo.size()!=1) {
    		
    		throw new PjException("该数据只能有一条");
		}
    	ReceiptAddressVo receiptAddressVo= listReceiptAddressVo.get(0);
...
//    	model.addAttribute("shipperList",shipperService.list(searchPara, "BASE_SHIPPER.ID desc" ));
    	model.addAttribute("bean", receiptAddressVo);
    	
    	return  "/yulinchemical/base/receiptaddress/toAddFreight";
