 ...
 */
package com.dfhc.base.logisticscompany.web;
 ...
 * insert page      GET  /logisticscompany/insert
 * insert action    POST /logisticscompany/insert
 * update page      GET  /logisticscompany/update/{id}
 * update action    POST /logisticscompany/update
 * delete action    POST /logisticscompany/delete
 * detail                /logisticscompany/detail/{id}

 * reference             /logisticscompany/reference
 * import page      GET  /logisticscompany/import
 * import action    POST /logisticscompany/import
 * export custom    GET  /logisticscompany/export
 * export action    POST /logisticscompany/export
 */
...
public class LogisticsCompanyController implements ILogisticsCompanyConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/logisticscompany/listLogisticsCompany";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, logisticsCompanyService.getCount(searchPara));
        ...
        List<LogisticsCompanyVo> beans = logisticsCompanyService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid LogisticsCompanyVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
//        logisticsCompanyService.insert(vo);  //插入单条记录
        logisticsCompanyService.insert(vo, request);
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid LogisticsCompanyVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
//      logisticsCompanyService.update(vo);  //更新单条记录
        logisticsCompanyService.update(vo,request); 
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
...
//        LogisticsCompanyVo bean = logisticsCompanyService.get(id);
    	LogisticsCompanyVo bean = logisticsCompanyService.get(id,request);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = logisticsCompanyService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += logisticsCompanyService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/logisticscompany?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
//            deleteCount = logisticsCompanyService.deleteLogic(request,new String(id));
        	deleteCount = logisticsCompanyService.doDelete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += logisticsCompanyService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/logisticscompany?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
...
//        String id = request.getParameter(REQUEST_ID);
//        if(id != null && id.length() > 0) {
//            deleteCount = logisticsCompanyService.doDelete(new String(id),request);
//        } else {
//            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
//            if (ids != null && ids.length != 0) {
//                deleteCount += logisticsCompanyService.deleteLogic(request,ids);  //删除多条记录
...
//    }
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/logisticscompany/detailLogisticsCompany";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/logisticscompany/util/referenceLogisticsCompany";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/logisticscompany/importLogisticsCompany";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/logisticscompany/exportLogisticsCompany_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/logisticscompany/exportLogisticsCompany_excel";
    }
