 ...
 */
package com.dfhc.base.logisticscompany.dao;
 ...
 */
@Repository
public class LogisticsCompanyDao extends RmSqlSessionDaoSupport implements ILogisticsCompanyConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<LogisticsCompanyVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<LogisticsCompanyVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           LogisticsCompanyVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
    ...
    public LogisticsCompanyVo getCompanyVoByName(String name){
    	return getSqlSession().selectOne(namespace("getByName"), name);
    }
}
