 ...
 *
 * 文件名称：  ShipperController.java
 *
 * 功能描述：  发货方维护Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.base.shipper.web;
...
/**
 * list                  /shipper
 * insert page      GET  /shipper/insert
 * insert action    POST /shipper/insert
 * update page      GET  /shipper/update/{id}
 * update action    POST /shipper/update
 * delete action    POST /shipper/delete
 * detail                /shipper/detail/{id}

 * reference             /shipper/reference
 * import page      GET  /shipper/import
 * import action    POST /shipper/import
 * export custom    GET  /shipper/export
 * export action    POST /shipper/export
 */
...
public class ShipperController implements IShipperConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        //是否默认
        model.addAttribute(ISystemConstant.DICTIONARY_RM_YES_NOT,RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT));
        model.addAttribute("status", RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT));
        
        return "/yulinchemical/base/shipper/listShipper";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, shipperService.getCount(searchPara));
        ...
        List<ShipperVo> beans = shipperService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid ShipperVo vo) {
        Map<String, Object> result = new HashMap<String, Object>();
        result.put("delete_flag",ISystemConstant.DICTIONARY_RM_YES_NOT_0);
        result.put("isDefault", ISystemConstant.DICTIONARY_RM_YES_NOT_1);
        List<ShipperVo>  list=shipperService.list(result, null);
    	if(list.isEmpty()){
        	RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
//            shipperService.insert(vo);  //插入单条记录
        	shipperService.insert(vo, request);
            ...
            result.put(ISystemConstant.AJAX_MESSAGE, "已有默认的数据");
    	}
        return result;
    }
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid ShipperVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        shipperService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        ShipperVo bean = shipperService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
            result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_FAIL);
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = shipperService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += shipperService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/shipper?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = shipperService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += shipperService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/shipper?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
     ...
     * @return
     */
    @RequestMapping(value="ajaxDeleteShipper",method=RequestMethod.POST,
    		consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    
    @ResponseBody
    public Map<String,Object> ajaxDeleteShipper(HttpServletRequest request,Model model){
    	...
    	Map<String,Object> result=new HashMap<String,Object>();
          ...
          int deleteCount = 0;  //定义成功删除的记录数
          if (id != null && id.length() > 0) {
        	  deleteCount = shipperService.deleteLogic(request,id);
          }else{
        	  String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/shipper/detailShipper";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        model.addAttribute("status", RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT));
        return "/yulinchemical/base/shipper/util/referenceShipper";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/shipper/importShipper";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/shipper/exportShipper_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/shipper/exportShipper_excel";
    }
