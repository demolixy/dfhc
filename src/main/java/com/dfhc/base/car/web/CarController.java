...
import com.dfhc.base.car.ICarConstants;
import com.dfhc.base.car.service.CarService;
import com.dfhc.base.car.vo.CarVo;
...
/**
 * list                  /car
 * insert page      GET  /car/insert
 * insert action    POST /car/insert
 * update page      GET  /car/update/{id}
 * update action    POST /car/update
 * delete action    POST /car/delete
 * detail                /car/detail/{id}

 * reference             /car/reference
 * import page      GET  /car/import
 * import action    POST /car/import
 * export custom    GET  /car/export
 * export action    POST /car/export
 */
...
public class CarController implements ICarConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/car/listCar";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, carService.getCount(searchPara));
        ...
        List<CarVo> beans = carService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid CarVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        carService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid CarVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        carService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = carService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += carService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/car?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = carService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += carService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/car?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/car/detailCar";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/car/util/referenceCar";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/car/importCar";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/car/exportCar_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/car/exportCar_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("grand", request.getParameter("grand"));
           searchMap.put("model", request.getParameter("model"));
           searchMap.put("color", request.getParameter("color"));
           searchMap.put("modelSerial", request.getParameter("modelSerial"));
           searchMap.put("engineNo", request.getParameter("engineNo"));
           searchMap.put("certDept", request.getParameter("certDept"));
           searchMap.put("emptyWeight", request.getParameter("emptyWeight"));
           searchMap.put("owner", request.getParameter("owner"));
           searchMap.put("driveNo", request.getParameter("driveNo"));
           searchMap.put("driveType", request.getParameter("driveType"));
           searchMap.put("contact", request.getParameter("contact"));
           searchMap.put("gender", request.getParameter("gender"));
           searchMap.put("idNo", request.getParameter("idNo"));
           searchMap.put("axleNum", request.getParameter("axleNum"));
           searchMap.put("picFront", request.getParameter("picFront"));
           searchMap.put("picBack", request.getParameter("picBack"));
