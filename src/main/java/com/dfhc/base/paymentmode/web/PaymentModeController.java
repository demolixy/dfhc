 ...
 *
 * 文件名称：  PaymentModeController.java
 *
 * 功能描述：  收款方式Controller
 * 
 * 版本历史：
 ...
 */
package com.dfhc.base.paymentmode.web;
...
import com.dfhc.base.paymentmode.service.PaymentModeService;
...
/**
 * list                  /paymentmode
 * insert page      GET  /paymentmode/insert
 * insert action    POST /paymentmode/insert
 * update page      GET  /paymentmode/update/{id}
 * update action    POST /paymentmode/update
 * delete action    POST /paymentmode/delete
 * detail                /paymentmode/detail/{id}

 * reference             /paymentmode/reference
 * import page      GET  /paymentmode/import
 * import action    POST /paymentmode/import
 * export custom    GET  /paymentmode/export
 * export action    POST /paymentmode/export
 */
...
public class PaymentModeController implements IPaymentModeConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/paymentmode/listPaymentMode";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, paymentModeService.getCount(searchPara));
        ...
        List<PaymentModeVo> beans = paymentModeService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid PaymentModeVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        paymentModeService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid PaymentModeVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        paymentModeService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        PaymentModeVo bean = paymentModeService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = paymentModeService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += paymentModeService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/paymentmode?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = paymentModeService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += paymentModeService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/paymentmode?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/paymentmode/detailPaymentMode";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/paymentmode/util/referencePaymentMode";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/paymentmode/importPaymentMode";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/paymentmode/exportPaymentMode_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/paymentmode/exportPaymentMode_excel";
    }
