 ...
 *
 * 文件名称：  WarehouseGateController.java
 *
 * 功能描述：  仓库门号Controller
 * 
 * 版本历史：
 ...
 */
package com.dfhc.base.warehousegate.web;
...
import com.dfhc.base.warehousegate.service.WarehouseGateService;
...
/**
 * list                  /warehousegate
 * insert page      GET  /warehousegate/insert
 * insert action    POST /warehousegate/insert
 * update page      GET  /warehousegate/update/{id}
 * update action    POST /warehousegate/update
 * delete action    POST /warehousegate/delete
 * detail                /warehousegate/detail/{id}

 * reference             /warehousegate/reference
 * import page      GET  /warehousegate/import
 * import action    POST /warehousegate/import
 * export custom    GET  /warehousegate/export
 * export action    POST /warehousegate/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/warehousegate")
public class WarehouseGateController implements IWarehouseGateConstants {

    @Autowired
    /**
     * 仓库门号服务
     */
    private WarehouseGateService warehouseGateService;
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehousegate/listWarehouseGate";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, warehouseGateService.getCount(searchPara));
        ...
        List<WarehouseGateVo> beans = warehouseGateService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid WarehouseGateVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        warehouseGateService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid WarehouseGateVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        warehouseGateService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        WarehouseGateVo bean = warehouseGateService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = warehouseGateService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += warehouseGateService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/warehousegate?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = warehouseGateService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += warehouseGateService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/warehousegate?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehousegate/detailWarehouseGate";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehousegate/util/referenceWarehouseGate";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/warehousegate/importWarehouseGate";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/warehousegate/exportWarehouseGate_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/warehousegate/exportWarehouseGate_excel";
    }
