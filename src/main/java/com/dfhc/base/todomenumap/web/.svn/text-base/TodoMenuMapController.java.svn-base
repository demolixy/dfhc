 ...
 *
 * 文件名称：  TodoMenuMapController.java
 *
 * 功能描述：  待办菜单映射表Controller
 * 
 * 版本历史：
 * 
 * 2017-03-13   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.base.todomenumap.web;
...
import com.dfhc.base.todomenumap.service.TodoMenuMapService;
...
/**
 * list                  /todomenumap
 * insert page      GET  /todomenumap/insert
 * insert action    POST /todomenumap/insert
 * update page      GET  /todomenumap/update/{id}
 * update action    POST /todomenumap/update
 * delete action    POST /todomenumap/delete
 * detail                /todomenumap/detail/{id}

 * reference             /todomenumap/reference
 * import page      GET  /todomenumap/import
 * import action    POST /todomenumap/import
 * export custom    GET  /todomenumap/export
 * export action    POST /todomenumap/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/todomenumap")
public class TodoMenuMapController implements ITodoMenuMapConstants {

    @Autowired
    /**
     * 待办菜单映射表服务
     */
    private TodoMenuMapService todoMenuMapService;
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/todomenumap/listTodoMenuMap";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, todoMenuMapService.getCount(searchPara));
        ...
        List<TodoMenuMapVo> beans = todoMenuMapService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid TodoMenuMapVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        todoMenuMapService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid TodoMenuMapVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        todoMenuMapService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        TodoMenuMapVo bean = todoMenuMapService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = todoMenuMapService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += todoMenuMapService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/todomenumap?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = todoMenuMapService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += todoMenuMapService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/todomenumap?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/todomenumap/detailTodoMenuMap";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/todomenumap/util/referenceTodoMenuMap";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/todomenumap/importTodoMenuMap";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/todomenumap/exportTodoMenuMap_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/todomenumap/exportTodoMenuMap_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("menuName", request.getParameter("menuName"));
           searchMap.put("menuSql", request.getParameter("menuSql"));
           searchMap.put("menuUrl", request.getParameter("menuUrl"));
   ...
   /**
    ...
    public String toDetail(Model model, HttpServletRequest request){    	
    	...
    	TodoMenuMapVo vo=todoMenuMapService.get(id);
    	
    	model.addAttribute("todoMenuMapVo", vo);
