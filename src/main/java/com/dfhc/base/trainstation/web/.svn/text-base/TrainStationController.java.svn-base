 ...
 *
 * 文件名称：  TrainStationController.java
 *
 * 功能描述：  火车站点Controller
 * 
 * 版本历史：
 ...
 */
package com.dfhc.base.trainstation.web;
...
/**
 * list                  /trainstation
 * insert page      GET  /trainstation/insert
 * insert action    POST /trainstation/insert
 * update page      GET  /trainstation/update/{id}
 * update action    POST /trainstation/update
 * delete action    POST /trainstation/delete
 * detail                /trainstation/detail/{id}

 * reference             /trainstation/reference
 * import page      GET  /trainstation/import
 * import action    POST /trainstation/import
 * export custom    GET  /trainstation/export
 * export action    POST /trainstation/export
 */
...
public class TrainStationController implements ITrainStationConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/trainstation/listTrainStation";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, trainStationService.getCount(searchPara));
        ...
        List<TrainStationVo> beans = trainStationService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid TrainStationVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        trainStationService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid TrainStationVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        trainStationService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        TrainStationVo bean = trainStationService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = trainStationService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += trainStationService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/trainstation?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = trainStationService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += trainStationService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/trainstation?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/trainstation/detailTrainStation";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/trainstation/util/referenceTrainStation";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/trainstation/importTrainStation";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/trainstation/exportTrainStation_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/trainstation/exportTrainStation_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("stationName", request.getParameter("stationName"));
     ...
     * @return
     */
    @RequestMapping(value="addFreight/{id}",method=RequestMethod.GET)
    public String addFreight(@PathVariable  String id ,HttpServletRequest request ,Model model){
        
    	model.addAttribute("statusType", RmGlobalReference.get(ISystemConstant.DICTIONARY_TARGET_TYPE));

    	if(StringHelper.isEmpty(id)){
    		
    		throw new PjException("没找到对应的火车站点id");
    	}
    	...
    	List<TrainStationVo> listTrainStationVo=trainStationService.getI(searchPara, "", 1, -1);
    	
    	if (listTrainStationVo.size()!=1) {
    		
    		throw new PjException("该数据只能有一条");
		}
    	TrainStationVo trainStationVo= listTrainStationVo.get(0);
    	
    	searchPara.clear();
    
    	model.addAttribute("bean", trainStationVo);
    	
    	return  "/yulinchemical/base/trainstation/addFreight";
