 ...
 *
 * 文件名称：  SecurityIndexController.java
 *
 * 功能描述：  安检指标表Controller
 * 
 * 版本历史：
 * 
 * 2017-04-01   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.base.securityindex.web;
...
/**
 * list                  /securityindex
 * insert page      GET  /securityindex/insert
 * insert action    POST /securityindex/insert
 * update page      GET  /securityindex/update/{id}
 * update action    POST /securityindex/update
 * delete action    POST /securityindex/delete
 * detail                /securityindex/detail/{id}

 * reference             /securityindex/reference
 * import page      GET  /securityindex/import
 * import action    POST /securityindex/import
 * export custom    GET  /securityindex/export
 * export action    POST /securityindex/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/securityindex")
public class SecurityIndexController implements ISecurityIndexConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/securityindex/listSecurityIndex";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, securityIndexService.getCount(searchPara));
        ...
        List<SecurityIndexVo> beans = securityIndexService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid SecurityIndexVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        securityIndexService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid SecurityIndexVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        securityIndexService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        SecurityIndexVo bean = securityIndexService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = securityIndexService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += securityIndexService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/securityindex?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = securityIndexService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += securityIndexService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/securityindex?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/securityindex/detailSecurityIndex";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/securityindex/util/referenceSecurityIndex";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/securityindex/importSecurityIndex";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/securityindex/exportSecurityIndex_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/securityindex/exportSecurityIndex_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("checkResultCtrlType", request.getParameter("checkResultCtrlType"));
   ...
   /**
    ...
    public String toDetail(Model model, HttpServletRequest request){    	
    	...
    	SecurityIndexVo vo=securityIndexService.get(id);
    	
    	model.addAttribute("securityIndexVo", vo);
