 ...
 */
package com.dfhc.base.shipmode.web;
...
/**
 * list                  /shipmode
 * insert page      GET  /shipmode/insert
 * insert action    POST /shipmode/insert
 * update page      GET  /shipmode/update/{id}
 * update action    POST /shipmode/update
 * delete action    POST /shipmode/delete
 * detail                /shipmode/detail/{id}

 * reference             /shipmode/reference
 * import page      GET  /shipmode/import
 * import action    POST /shipmode/import
 * export custom    GET  /shipmode/export
 * export action    POST /shipmode/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/shipmode")
public class ShipModeController implements IShipModeConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/shipmode/listShipMode";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, shipModeService.getCount(searchPara));
        ...
        List<ShipModeVo> beans = shipModeService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid ShipModeVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        shipModeService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid ShipModeVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        shipModeService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        ShipModeVo bean = shipModeService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = shipModeService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += shipModeService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/shipmode?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = shipModeService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += shipModeService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/shipmode?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/shipmode/detailShipMode";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        model.addAttribute("status", RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT));
        return "/yulinchemical/base/shipmode/util/referenceShipMode";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/shipmode/importShipMode";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/shipmode/exportShipMode_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/shipmode/exportShipMode_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("isTray", request.getParameter("isTray"));
