 ...
 *
 * 文件名称：  WarehousePositionController.java
 *
 * 功能描述：  仓位号Controller
 * 
 * 版本历史：
 ...
 */
package com.dfhc.base.warehouseposition.web;
...
import com.dfhc.base.warehouseposition.service.WarehousePositionService;
...
/**
 * list                  /warehouseposition
 * insert page      GET  /warehouseposition/insert
 * insert action    POST /warehouseposition/insert
 * update page      GET  /warehouseposition/update/{id}
 * update action    POST /warehouseposition/update
 * delete action    POST /warehouseposition/delete
 * detail                /warehouseposition/detail/{id}

 * reference             /warehouseposition/reference
 * import page      GET  /warehouseposition/import
 * import action    POST /warehouseposition/import
 * export custom    GET  /warehouseposition/export
 * export action    POST /warehouseposition/export
 */
...
public class WarehousePositionController implements IWarehousePositionConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehouseposition/listWarehousePosition";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, warehousePositionService.getCount(searchPara));
        ...
        List<WarehousePositionVo> beans = warehousePositionService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid WarehousePositionVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        warehousePositionService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid WarehousePositionVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        warehousePositionService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        WarehousePositionVo bean = warehousePositionService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = warehousePositionService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += warehousePositionService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/warehouseposition?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = warehousePositionService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += warehousePositionService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/warehouseposition?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehouseposition/detailWarehousePosition";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/warehouseposition/util/referenceWarehousePosition";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/warehouseposition/importWarehousePosition";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/warehouseposition/exportWarehousePosition_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/warehouseposition/exportWarehousePosition_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("warehouseGateId", request.getParameter("warehouseGateId"));
           searchMap.put("name", request.getParameter("name"));
           searchMap.put("isEnable", request.getParameter("isEnable"));
