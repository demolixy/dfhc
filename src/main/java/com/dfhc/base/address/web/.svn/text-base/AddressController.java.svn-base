 ...
 */
package com.dfhc.base.address.web;
...
import com.dfhc.base.address.IAddressConstants;
import com.dfhc.base.address.service.AddressService;
import com.dfhc.base.address.vo.AddressVo;
...
/**
 * list                  /address
 * insert page      GET  /address/insert
 * insert action    POST /address/insert
 * update page      GET  /address/update/{id}
 * update action    POST /address/update
 * delete action    POST /address/delete
 * detail                /address/detail/{id}

 * reference             /address/reference
 * import page      GET  /address/import
 * import action    POST /address/import
 * export custom    GET  /address/export
 * export action    POST /address/export
 */
...
public class AddressController implements IAddressConstants {
	 ...
	 * @return
	 */
	@RequestMapping(value = "addressShow/{id}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public Map<String, Object> getAddress(@PathVariable int id,
			HttpServletRequest request) {
		List<AddressVo> addressVos = addressService.getAddressForCondition(id);
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("addressVos", addressVos);
		return result;
	}
	/**
	 * 三级联动查询省市区
	 ...
	 * @return
	 */
	@RequestMapping(value = "getAddressCompany/{id}", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseBody
	public Map<String, Object> getAddressCompany(@PathVariable int id,
			HttpServletRequest request) {
		List<AddressVo> addressVos = addressService.getAddressForCondition(id);
		Map<String, Object> result = new HashMap<String, Object>();
		result.put("addressVos", addressVos);
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/address/listAddress";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, addressService.getCount(searchPara));
        ...
        List<AddressVo> beans = addressService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid AddressVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        addressService.insert(vo);  //插入单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
        result.put(ISystemConstant.AJAX_MESSAGE, "新增成功: " + vo.getAddressId());
        return result;
    }
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid AddressVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        addressService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
        result.put(ISystemConstant.AJAX_MESSAGE, "修改成功: " + vo.getAddressId());
        return result;
    }
    /**
     * 根据id获取单条记录
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        AddressVo bean = addressService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = addressService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += addressService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/address?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = addressService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += addressService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/address?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/address/detailAddress";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/base/address/util/referenceAddress";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/base/address/importAddress";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/base/address/exportAddress_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/base/address/exportAddress_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("addressId", request.getParameter("addressId"));
           searchMap.put("parentId", request.getParameter("parentId"));
           searchMap.put("addressName", request.getParameter("addressName"));
           searchMap.put("priority", request.getParameter("priority"));
           searchMap.put("isEnabled", request.getParameter("isEnabled"));
    ...
    public Map<String, Object> getAreaInfo(Model model,HttpServletRequest request,HttpServletResponse response) throws Exception {    
       String type = request.getParameter("type");
       String ts = null;
       String sel = null;
       if("province".equals(type)){
    	   ts = "<option value=\"\">--请选择省--</option>";
    	   sel = "${member.provinceId=='";
       }else if("city".equals(type)){
    	   ts = "<option value=\"\">--请选择市--</option>";
    	   sel = "${member.cityId=='";
       }else{
    	   ts = "<option value=\"\">--请选择区县--</option>";
    	   sel = "${member.countyId=='";
       }
       String prev =request.getParameter("prev");
       List<RmCommonVo> list = addressService.getAreaInfo(prev);
       response.setContentType("text/html;charset=UTF-8");
       PrintWriter out = response.getWriter();
       StringBuilder selectHtml = new StringBuilder();
       if(list!=null && list.size()>0){
    	   selectHtml.append(ts);
    	   for(RmCommonVo comVo : list){
    		   selectHtml.append("<option value=\""+comVo.getString("address_id")+"\">"+comVo.getString("address_name")+"</option>");
    		   /*selectHtml.append("<option value=\"");
    		   selectHtml.append(comVo.getString("address_id")+"\" <c:if test=\"");
    		   selectHtml.append(sel+comVo.getString("address_id")+"'}\">selected=\"");
    		   selectHtml.append("selected");
    		   selectHtml.append("</c:if>");
    		   selectHtml.append(">");
    		   selectHtml.append(comVo.getString("address_name")+"</option>");
    		   System.out.println(selectHtml.toString());*/
    	   }
    	   sel=null;
       }
