 ...
 */
package com.dfhc.base.address.dao;
 ...
 */
@Repository
public class AddressDao extends RmSqlSessionDaoSupport implements IAddressConstants {
    ...
    public String insert(AddressVo vo) {
        if(vo.getAddressId() == null || vo.getAddressId().length() == 0) {
            vo.setAddressName(RmIdFactory.requestId(TABLE_NAME)); //获得id
        }
        getSqlSession().insert(namespace("insert"), vo);
        return vo.getAddressId();
    }
     ...
     * @return 查询到的VO列表
     */
    public List<AddressVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<AddressVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
            AddressVo vo = vos.get(i);
            if(vo.getAddressId() == null || vo.getAddressId().length() == 0) {
              vo.setAddressName(idList.remove(0)); //获得id
           }
        }
