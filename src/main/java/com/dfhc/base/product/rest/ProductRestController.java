 ...
 *
 * 文件名称：  ProductRestController.java
 *
 * 功能描述：  产品管理 rest controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.base.product.rest;
...
import com.dfhc.base.product.web.ProductController;
 ...
 * insert action        POST    /api/product
 * update action        PUT     /api/product/{id}
 * delete action        DELETE  /api/product/{id}
 * delete multi action  POST    /api/product/delete
 * batch insert update  POST    /api/product/batch
 */
...
public class ProductRestController implements IProductConstants {
    ...
    public Map<String, Object> list(HttpServletRequest request) {
        Map<String, Object> searchPara = ProductController.getQueryCondition(request);  //从request中获得查询条件
    ...
    public ResponseEntity<?> get(@PathVariable("id") String id) {
        ProductVo task = productService.get(id);
        if (task == null) {
            return new ResponseEntity<ProductVo>(HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<ProductVo>(task, HttpStatus.OK);
    }
    ...
    @ResponseBody
    public ResponseEntity<?> insert(@RequestBody ProductVo vo, UriComponentsBuilder uriBuilder) {
        //调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
        Set<ConstraintViolation<ProductVo>> failures = validator.validate(vo);
        if (!failures.isEmpty()) {
        ...
        String id = vo.getId();
        URI uri = uriBuilder.path("/api/product/" + id).build().toUri();
    ...
    public int delete(HttpServletRequest request) {
        ...
        if (ids != null && ids.length != 0) {
            return productService.delete(ids); //删除多条记录
        }
        return 0;
    }
    ...
    public ResponseEntity<Map<String, int[]>> batchInsertUpdate(HttpServletRequest request) {
        List<ProductVo> lvo = RmPopulateHelper.populateAjax(ProductVo.class, request);
        for(ProductVo vo : lvo) {
            if(vo.getId() != null) {
        ...
        int[] sum_insert_update = productService.insertUpdateBatch(lvo.toArray(new ProductVo[0]));
