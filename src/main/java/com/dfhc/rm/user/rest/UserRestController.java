 ...
 * 系统名称：  activiti工作流
 *
 * 文件名称：  UserRestController.java
 *
 * 功能描述：  RM_USER rest controller
 * 
 * 版本历史：
 * 
 * 2016-12-07   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.rm.user.rest;
...
import com.dfhc.rm.user.web.UserController;
 ...
 * list                 GET     /api/user
 * get                  GET     /api/user/{id}
 * insert action        POST    /api/user
 * update action        PUT     /api/user/{id}
 * delete action        DELETE  /api/user/{id}
 * delete multi action  POST    /api/user/delete
 * batch insert update  POST    /api/user/batch
 */

/**
 * RM_USERrest controller
 * 
 * @author 龙色波
 * @see 参见的类
 */
...
public class UserRestController implements IUserConstants {
    ...
    public Map<String, Object> list(HttpServletRequest request) {
        Map<String, Object> searchPara = UserController.getQueryCondition(request);  //从request中获得查询条件
    ...
    public ResponseEntity<?> get(@PathVariable("id") String id) {
        UserVo task = userService.get(id);
        if (task == null) {
            return new ResponseEntity<UserVo>(HttpStatus.NOT_FOUND);
        }
        return new ResponseEntity<UserVo>(task, HttpStatus.OK);
    }
    ...
    @ResponseBody
    public ResponseEntity<?> insert(@RequestBody UserVo vo, UriComponentsBuilder uriBuilder) {
        //调用JSR303 Bean Validator进行校验，如果出错返回含400错误码及json格式的错误信息.
        Set<ConstraintViolation<UserVo>> failures = validator.validate(vo);
        if (!failures.isEmpty()) {
        ...
        String id = vo.getId();
        URI uri = uriBuilder.path("/api/user/" + id).build().toUri();
    ...
    public int delete(HttpServletRequest request) {
        ...
        if (ids != null && ids.length != 0) {
            return userService.delete(ids); //删除多条记录
        }
        return 0;
    }
    ...
    public ResponseEntity<Map<String, int[]>> batchInsertUpdate(HttpServletRequest request) {
        List<UserVo> lvo = RmPopulateHelper.populateAjax(UserVo.class, request);
        for(UserVo vo : lvo) {
            if(vo.getId() != null) {
        ...
        int[] sum_insert_update = userService.insertUpdateBatch(lvo.toArray(new UserVo[0]));
