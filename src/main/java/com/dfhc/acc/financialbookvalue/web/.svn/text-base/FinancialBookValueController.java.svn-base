 ...
 *
 * 文件名称：  FinancialBookValueController.java
 *
 * 功能描述：  客户财务账面金额Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.acc.financialbookvalue.web;
...
/**
 * list                  /financialbookvalue
 * insert page      GET  /financialbookvalue/insert
 * insert action    POST /financialbookvalue/insert
 * update page      GET  /financialbookvalue/update/{id}
 * update action    POST /financialbookvalue/update
 * delete action    POST /financialbookvalue/delete
 * detail                /financialbookvalue/detail/{id}

 * reference             /financialbookvalue/reference
 * import page      GET  /financialbookvalue/import
 * import action    POST /financialbookvalue/import
 * export custom    GET  /financialbookvalue/export
 * export action    POST /financialbookvalue/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/financialbookvalue")
public class FinancialBookValueController implements IFinancialBookValueConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/financialbookvalue/listFinancialBookValue";
    }
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid FinancialBookValueVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        financialBookValueService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid FinancialBookValueVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        financialBookValueService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        FinancialBookValueVo bean = financialBookValueService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = financialBookValueService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += financialBookValueService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/financialbookvalue?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = financialBookValueService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += financialBookValueService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/financialbookvalue?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/financialbookvalue/detailFinancialBookValue";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/financialbookvalue/util/referenceFinancialBookValue";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/acc/financialbookvalue/importFinancialBookValue";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/acc/financialbookvalue/exportFinancialBookValue_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/acc/financialbookvalue/exportFinancialBookValue_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("balance", request.getParameter("balance"));
