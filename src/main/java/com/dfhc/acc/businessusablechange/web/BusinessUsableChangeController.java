 ...
 *
 * 文件名称：  BusinessUsableChangeController.java
 *
 * 功能描述：  业务可用额变动Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.acc.businessusablechange.web;
...
import com.dfhc.acc.businessusablechange.IBusinessUsableChangeConstants;
import com.dfhc.acc.businessusablechange.service.BusinessUsableChangeService;
...
/**
 * list                  /businessusablechange
 * insert page      GET  /businessusablechange/insert
 * insert action    POST /businessusablechange/insert
 * update page      GET  /businessusablechange/update/{id}
 * update action    POST /businessusablechange/update
 * delete action    POST /businessusablechange/delete
 * detail                /businessusablechange/detail/{id}

 * reference             /businessusablechange/reference
 * import page      GET  /businessusablechange/import
 * import action    POST /businessusablechange/import
 * export custom    GET  /businessusablechange/export
 * export action    POST /businessusablechange/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/businessusablechange")
public class BusinessUsableChangeController implements IBusinessUsableChangeConstants {

    @Autowired
    /**
     * 业务可用额变动服务
     */
    private BusinessUsableChangeService businessUsableChangeService;
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/businessusablechange/listBusinessUsableChange";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, businessUsableChangeService.getCount(searchPara));
        ...
        List<BusinessUsableChangeVo> beans = businessUsableChangeService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid BusinessUsableChangeVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        businessUsableChangeService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid BusinessUsableChangeVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        businessUsableChangeService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        BusinessUsableChangeVo bean = businessUsableChangeService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = businessUsableChangeService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += businessUsableChangeService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/businessusablechange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = businessUsableChangeService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += businessUsableChangeService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/businessusablechange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/businessusablechange/detailBusinessUsableChange";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/businessusablechange/util/referenceBusinessUsableChange";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/acc/businessusablechange/importBusinessUsableChange";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/acc/businessusablechange/exportBusinessUsableChange_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/acc/businessusablechange/exportBusinessUsableChange_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("businessAvailabilityId", request.getParameter("businessAvailabilityId"));
           searchMap.put("customerId", request.getParameter("customerId"));
           searchMap.put("customerName", request.getParameter("customerName"));
           searchMap.put("oldAvailableCredit", request.getParameter("oldAvailableCredit"));
           searchMap.put("availableCreditChange", request.getParameter("availableCreditChange"));
           searchMap.put("oldFreezingAmount", request.getParameter("oldFreezingAmount"));
           searchMap.put("freezingAccrual", request.getParameter("freezingAccrual"));
           searchMap.put("nowAvailableCredit", request.getParameter("nowAvailableCredit"));
           searchMap.put("digest", request.getParameter("digest"));
           searchMap.put("amountDirection", request.getParameter("amountDirection"));
           searchMap.put("nowFreezingAmount", request.getParameter("nowFreezingAmount"));
           searchMap.put("relevanceBillsId", request.getParameter("relevanceBillsId"));
