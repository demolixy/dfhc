 ...
 *
 * 文件名称：  ReceiptConfLetterDao.java
 *
 * 功能描述：  收货确认函数据访问对象
 * 
 * 版本历史：
 ...
 */
package com.dfhc.acc.receiptconfletter.dao;
 ...
 */
@Repository
public class ReceiptConfLetterDao extends RmSqlSessionDaoSupport implements IReceiptConfLetterConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<ReceiptConfLetterVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<ReceiptConfLetterVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           ReceiptConfLetterVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
