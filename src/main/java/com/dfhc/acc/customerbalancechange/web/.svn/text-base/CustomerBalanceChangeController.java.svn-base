 ...
 *
 * 文件名称：  CustomerBalanceChangeController.java
 *
 * 功能描述：  客户余额变动Controller
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.acc.customerbalancechange.web;
...
import com.dfhc.acc.customerbalancechange.ICustomerBalanceChangeConstants;
import com.dfhc.acc.customerbalancechange.service.CustomerBalanceChangeService;
...
/**
 * list                  /customerbalancechange
 * insert page      GET  /customerbalancechange/insert
 * insert action    POST /customerbalancechange/insert
 * update page      GET  /customerbalancechange/update/{id}
 * update action    POST /customerbalancechange/update
 * delete action    POST /customerbalancechange/delete
 * detail                /customerbalancechange/detail/{id}

 * reference             /customerbalancechange/reference
 * import page      GET  /customerbalancechange/import
 * import action    POST /customerbalancechange/import
 * export custom    GET  /customerbalancechange/export
 * export action    POST /customerbalancechange/export
 */
...
public class CustomerBalanceChangeController implements ICustomerBalanceChangeConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/customerbalancechange/listCustomerBalanceChange";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, customerBalanceChangeService.getCount(searchPara));
        ...
        List<CustomerBalanceChangeVo> beans = customerBalanceChangeService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid CustomerBalanceChangeVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        customerBalanceChangeService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid CustomerBalanceChangeVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        customerBalanceChangeService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        CustomerBalanceChangeVo bean = customerBalanceChangeService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = customerBalanceChangeService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += customerBalanceChangeService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/customerbalancechange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = customerBalanceChangeService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += customerBalanceChangeService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/customerbalancechange?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/customerbalancechange/detailCustomerBalanceChange";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/customerbalancechange/util/referenceCustomerBalanceChange";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/acc/customerbalancechange/importCustomerBalanceChange";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/acc/customerbalancechange/exportCustomerBalanceChange_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/acc/customerbalancechange/exportCustomerBalanceChange_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("financialBookValueId", request.getParameter("financialBookValueId"));
           searchMap.put("customerId", request.getParameter("customerId"));
           searchMap.put("customerName", request.getParameter("customerName"));
           searchMap.put("oldBalance", request.getParameter("oldBalance"));
           searchMap.put("accrual", request.getParameter("accrual"));
           searchMap.put("nowBalance", request.getParameter("nowBalance"));
