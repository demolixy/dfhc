 ...
 *
 * 文件名称：  LogisticsCostSettlementDao.java
 *
 * 功能描述：  物流费用结算审批单数据访问对象
 * 
 * 版本历史：
 * 
 * 2017-01-04   1.0.0版 （龙色波）（创建文件）
 */
package com.dfhc.acc.logisticscostsettlement.dao;
 ...
 */
@Repository
public class LogisticsCostSettlementDao extends RmSqlSessionDaoSupport implements ILogisticsCostSettlementConstants {
     ...
     * @return 查询到的VO列表
     */
    public List<LogisticsCostSettlementVo> list(Map<String, Object> searchPara, String orderStr, int startIndex, int size, boolean allClumn) {
    ...
    public void insertBatch(List<LogisticsCostSettlementVo> vos) {
        ...
        for(int i=0;i<vos.size();i++) {
           LogisticsCostSettlementVo vo = vos.get(i);
           if(vo.getId() == null || vo.getId().length() == 0) {
