 ...
 *
 * 文件名称：  CreditManagerController.java
 *
 * 功能描述：  授信管理Controller
 * 
 * 版本历史：
 ...
 */
package com.dfhc.acc.creditmanager.web;
...
/**
 * list                  /creditmanager
 * insert page      GET  /creditmanager/insert
 * insert action    POST /creditmanager/insert
 * update page      GET  /creditmanager/update/{id}
 * update action    POST /creditmanager/update
 * delete action    POST /creditmanager/delete
 * detail                /creditmanager/detail/{id}

 * reference             /creditmanager/reference
 * import page      GET  /creditmanager/import
 * import action    POST /creditmanager/import
 * export custom    GET  /creditmanager/export
 * export action    POST /creditmanager/export
 */
 ...
 */
@Controller
@RequestMapping(value = "/creditmanager")
public class CreditManagerController implements ICreditManagerConstants {
    ...
    public String list(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/creditmanager/listCreditManager";
    }
    ...
    public void ajaxList(Model model, HttpServletRequest request, HttpServletResponse response) throws IOException{
        ...
        RmPageVo pageVo = RmJspHelper.transctPageVo(request, creditManagerService.getCount(searchPara));
        ...
        List<CreditManagerVo> beans = creditManagerService.list(searchPara, orderStr, pageVo.getStartIndex(), pageVo.getPageSize());  //按条件查询全部,带排序
    ...
    @ResponseBody
    public Map<String,Object> insert(HttpServletRequest request, @Valid CreditManagerVo vo) {
        RmVoHelper.markCreateStamp(request,vo);  //打创建时间,IP戳
        creditManagerService.insert(vo);  //插入单条记录
    ...
    @RequestMapping(value = "update", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> update(HttpServletRequest request, @Valid CreditManagerVo vo) {
        RmVoHelper.markModifyStamp(request,vo);  //打修改时间,IP戳
        creditManagerService.update(vo);  //更新单条记录
        Map<String, Object> result = new HashMap<String, Object>();
        result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
    ...
    public Map<String,Object> get(@PathVariable("id") String id, HttpServletRequest request) {
        CreditManagerVo bean = creditManagerService.get(id);
        Map<String, Object> result = new HashMap<String, Object>();
        if(bean==null){
    ...
    public String delete(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        if(id != null && id.length() > 0) {
            deleteCount = creditManagerService.delete(new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += creditManagerService.delete(ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/creditmanager?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    }
   /**
    ...
    public String deleteLogic(HttpServletRequest request, RedirectAttributes redirectAttributes) {
        ...
        String id = request.getParameter(REQUEST_ID);
        if(id != null && id.length() > 0) {
            deleteCount = creditManagerService.deleteLogic(request,new String(id));
        } else {
            String[] ids = RmJspHelper.getArrayFromRequest(request, REQUEST_IDS); //从request获取多条记录id
            if (ids != null && ids.length != 0) {
                deleteCount += creditManagerService.deleteLogic(request,ids);  //删除多条记录
            }
        }
        redirectAttributes.addFlashAttribute("message", "删除成功: " + deleteCount);
        return "redirect:/creditmanager?"+ISystemConstant.RM_PARENT_CODE+"="+request.getParameter(ISystemConstant.RM_PARENT_CODE);
    ...
    public String detail(@PathVariable("id") String id, Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/creditmanager/detailCreditManager";
    }
    ...
    public String reference(Model model, HttpServletRequest request) {
        ...
        return "/yulinchemical/acc/creditmanager/util/referenceCreditManager";
    }
    /**
     * 参照改为ajax
     * @param model
    ...
    public String importDataForm(Model model) {
        return "/yulinchemical/acc/creditmanager/importCreditManager";
    }
    ...
    public String exportCustomForm(Model model) {
        return "/yulinchemical/acc/creditmanager/exportCreditManager_custom";
    }
    ...
    public String exportData(Model model) {
        return "/yulinchemical/acc/creditmanager/exportCreditManager_excel";
    }
    ...
    public static Map<String, Object> getQueryCondition(HttpServletRequest request) {
        ...
        if(request.getAttribute(REQUEST_QUERY_CONDITION) != null) {  //如果request.getAttribute中有，就不取request.getParameter
           ...
           searchMap.put("lineOfCredit", request.getParameter("lineOfCredit"));
           searchMap.put("isStatus", request.getParameter("isStatus"));
    ...
    @RequestMapping(value = "insertOne", method = RequestMethod.POST, 
			consumes = MediaType.APPLICATION_FORM_URLENCODED_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    @ResponseBody
    public Map<String,Object> insertOne(HttpServletRequest request, @Valid CreditManagerVo vo) {
        Map<String, Object> result = new HashMap<String, Object>();
        try {
            creditManagerService.insertOne(request,vo);  //插入单条记录
            result.put(ISystemConstant.AJAX_STATUS, ISystemConstant.AJAX_RESULT_SUCCESS);
