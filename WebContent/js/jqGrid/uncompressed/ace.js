...
ace.handle_side_menu = function($) {
	...
	$('.nav-list').on(ace.click_event, function(e){
		...
		var pointer=$(link_element).parent("li");
		
		var childFlag=$(pointer).attr("childFlag");
		if(childFlag){//有值说明是子节点
			
			
		}else{//父节点
			var reqFlag=$(pointer).attr("reqFlag");
			var total_code=  $(pointer).attr("id");
			if(!reqFlag){
				ace.showChildTree($,pointer,total_code);
			}
...
//20161230，点击父节点，根据父节点的total_code，去请求
ace.showChildTree=function($,pointer,total_code){
	 $.ajax({
		    type:'post',//可选get
		    url:getRootPath()+'/pubController/getTreeOrgAjax',
		    data:{'total_code':total_code},
		    dataType:'json',
		    async:false , //false为同步
		    success:function(data,status){
			 var lvos=data.lvos;
			 var inHtml='<ul class="submenu">';
			 if(lvos.length==0){
				  $(pointer).append("<ul class='submenu'><li>无数据</li></ul>");
	              $(pointer).attr("reqFlag",true);//设置标志已经加载过了
				 }else{
              
                 for(var i=0;i<lvos.length;i++){
                	 var oul=lvos[i].data_value;
                	 var link_url="?";
                	 if(oul.indexOf("?")>-1){
                		 link_url="&";
                	 }
                
                	 var  childSource="ch"+i+"code";
                	 inHtml+='<li childFlag=true><a href="'+getRootPath()+oul+link_url+'pcode='+total_code+'&childSource='+childSource+'"><i class="icon-double-angle-right"></i>'	 
                	 
             	     inHtml+=lvos[i].name+"</a></li>";
                   }
                 inHtml+="</ul>";
                 $(pointer).append(inHtml);
                 $(pointer).attr("reqFlag",true);//设置标志已经加载过了
