...
//qb4.0旧版
function getReferenceOld(inputArray, path, urlPath, width, height){
	if(path == undefined) {
		path = dir_base;
	}
	if(width == undefined) {
	...
	//urlPath放最后
	myObject['urlPath'] = urlPath;
	var rtObj = window.showModalDialog(path + 'jsp/support/globalReferenceOld.jsp', myObject, 'dialogHeight=' + height + 'px;dialogWidth=' + width + 'px;');
	toDoWriteReference(inputArray, rtObj);
}
...
//获取组织结构参照5.0使用open
function getPartyWindowOpen(inputArray, path, urlPath, width, height) {
	if(path == undefined) {
		path = "../../";
	}
	if(width == undefined) {
	...
	obj.name = myObject;
	myObject.oldData = inputArray;
	var myWindow =window.open (urlPath,'newwindow','height=500,width=400,top=0,left=0,toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no') 
    //var rtObj = window.showModalDialog(urlPath, myObject, 'dialogHeight=' + height + 'px;dialogWidth=' + width + 'px;');
    var rtObj = $("#childName").val();
...
function writeBackValue(inputName) {
	...
	jQuery(":input[name='" + inputName + "']", document.forms[document.forms.length-1]).each(function() {
