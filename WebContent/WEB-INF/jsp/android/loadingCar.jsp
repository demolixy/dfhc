    ...
    <!--begin 表单元素3-->
     ...
     <!--begin 表单元素3-->
        <div class="dj_item">
          ...
          <div class="item_text">
         	<input type="number" name="truckInputBreakBagNum" placeholder="请输入破袋数量"/>
          </div>          
        </div>
     <!--end 表单元素3-->
     <!--begin 表单元素3-->
        <div class="dj_item">
          ...
          <div class="item_text">
         	<input type="number"  name="attribute1" placeholder="请输入序号"/>
          </div>          
        </div>
     <!--end 表单元素3-->
     <!--begin 表单元素3-->
        <div class="dj_item">
          ...
          <div class="item_text">
         	<input type="text"  name="attribute2" placeholder="请输入补袋人"/>
          </div>          
        </div>
     <!--end 表单元素3-->
...
<!--       <a href="javascript:void();" onclick="addBreakBag();" class="big_btn">+添加破袋数量</a> -->
 ...
 <!--end 表单内容-->
<script type="text/javascript">
// function addBreakBag(){
// 	$(".dj_list").find(".dj_item:last").after($('script[type="text/template"]').html());
// }
...
$(function(){
	$(".big_btn").click(function(){
		$.ajax({
	        type: "POST",
	        url: "<%=request.getContextPath()%>/api/loadingnotice/driverLoadingCar?date="+new Date(),
	        data:$(form).serialize(),		 
	        dataType: "json",
	        ...
	        success: function(data,status) {
		   	   if(data.status=="0"){
		   		...
		   		zfalert('系统提示',data.message,function(){ 
			          $(this).hide();
	    	   		 window.location = "<%=request.getContextPath()%>/api/loadingnotice/forklift?operation=08";
			    });
		   	   }
	       },
