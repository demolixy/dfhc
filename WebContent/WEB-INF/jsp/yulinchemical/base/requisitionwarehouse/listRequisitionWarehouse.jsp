...
function _saveInsert(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/requisitionwarehouse/insert',$(insertForm).serialize());
...
function _saveUpdate(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/requisitionwarehouse/update',$(updateForm).serialize());
}
</script>
</head>
<body>
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullX" name="name"  type="text"  						maxLength="200" />
							<i class="notNull">*</i>	
							</div>
							...
							<span class="add-on">省</span>
								<select name="provinceId" style="min-width: 200px;" >
									...
									<input class="m-wrap " name="province" value="" type="hidden"/>
							...
							<span class="add-on">市</span>
								<select name="cityId"  style="min-width: 200px;" >
									...
									<input class="m-wrap notNullX" name="city" value="" type="hidden"/>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("address")%></span>
							  <input class="m-wrap" name="address"  type="text"  						maxLength="300" />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on">流向名称</span>
							  <input type="text" class="m-wrap"  name="flowtoName"  readOnly=true/>						
							  <input type="hidden" class="m-wrap"  name="flowToId" />
								<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/flowto/reference?referenceInputType=radio','insertForm.flowToId,insertForm.flowtoName');">选择</span>
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contacts")%></span><input class="m-wrap" name="contacts"  type="text"  						maxLength="200" />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contactPhone")%></span><input class="m-wrap" name="contactPhone"  type="text"  						maxLength="20" />
							</div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullY" name="name" id="name" type="text"  						maxLength="200" />
							<i class="notNull">*</i>
							</div>
							...
							<span class="add-on">市</span>
								<select name="cityId"  style="min-width: 200px;" >
									...
									<input class="m-wrap " name="city" value="" type="hidden"/>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("address")%></span><input class="m-wrap" name="address" id="address" type="text"  						maxLength="300" />
							</div>
							...
							<div class="input-prepend">
							  <span class="add-on">流向名称</span>
							   <input type="text" class="m-wrap"  name="flowtoName"  readOnly=true/>						
							  <input type="hidden" class="m-wrap"  name="flowToId" />
							<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/flowto/reference?referenceInputType=radio','updateForm.flowToId,updateForm.flowtoName');">选择</span>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contacts")%></span><input class="m-wrap" name="contacts" id="contacts" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contactPhone")%></span><input class="m-wrap" name="contactPhone" id="contactPhone" type="text"  						maxLength="20" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/requisitionwarehouse/update',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="200" />
							</div>
						
					<%-- <div class="input-prepend">
							  <span class="add-on">流向名称</span>
							  <input type="text" class="m-wrap"  name="flowtoName" />						
							  <input type="hidden" class="m-wrap"  name="flowToId" />
							<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/flowto/reference?referenceInputType=radio','form.flowToId,form.flowToName');">选择</span>
							</div>    --%>		
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/requisitionwarehouse/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/requisitionwarehouse/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("province")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("provinceId")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("city")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("cityId")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("districtCounty")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("districtCountyId")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("address")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("flowToId")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contacts")%>'
,'<%=IRequisitionWarehouseConstants.TABLE_COLUMN_DISPLAY.get("contactPhone")%>'
],
					colModel:[
			...
			{name:'flowToId',index:'flowToId', width:90,editable: true,hidedlg:true,hidden:true,editoptions:{size:"20",maxLength:"19"}},
			...
			{name:'contactPhone',index:'contactPhone', width:90,editable: true,editoptions:{size:"20",maxLength:"20"}}
					...
					editurl: getRootPath()+'/requisitionwarehouse/update',//nothing is saved
					caption: "调拨库列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//运费维护页面
			     ...
			     function toFreight(){
			    	 ...
			    	 window.location.href="${pageContext.request.contextPath}/requisitionwarehouse/toFreight/"+id.toString();
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/requisitionwarehouse/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
                      ...
                      $("#portlet-config-edit").find("select[id=province]").val(data.bean.province);
                      $("#portlet-config-edit").find("input[id=provinceId]").val(data.bean.provinceId);
                      $("#portlet-config-edit").find("select[id=city]").val(data.bean.city);
                      $("#portlet-config-edit").find("input[id=cityId]").val(data.bean.cityId);
                      $("#portlet-config-edit").find("select[id=districtCounty]").val(data.bean.districtCounty);
                      $("#portlet-config-edit").find("input[id=districtCountyId]").val(data.bean.districtCountyId);
                      $("#portlet-config-edit").find("input[id=address]").val(data.bean.address);
                      $("#portlet-config-edit").find("input[name=flowtoName]").val(data.bean.flowtoName);
				...
				$("select[name=cityId]").change(function(){
					...
					$.ajax({
			            type: "POST",
			            url: "<%=request.getContextPath()%>/address/getAddressCompany/" + cityId,
			            dataType: "json",
			            async:true , //false为同步
			            success: function(data,status) {
							var addressVos = data.addressVos;
				...
				$("select[name=districtCountyId]").change(function(){
					$("input[name=districtCounty]").val($(this).find("option:selected").text());
				})
			});
		</script>
	</body>
