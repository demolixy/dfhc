<%--
   ...
   *
   * 文件名称：referenceReceiptAddress.jsp
   *
   * 功能描述：收货地址维护参照页面
   * 
   * 版本历史：
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("userId")%></span><input type="text" class="text_field_reference text_field_readonly"  hiddenInputId="userId" name="userIdName" inputName="<%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("userId")%>" value="" /><input type="hidden" name="userId"><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onclick="javascript:getReference(new Array(form.userId, form.userIdName), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/user/reference?referenceInputType=radio');"/>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("province")%></span><input class="m-wrap" name="province" id="province" type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("provinceId")%></span><input class="m-wrap" name="provinceId" id="provinceId" type="text"  						maxLength="10" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("city")%></span><input class="m-wrap" name="city" id="city" type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("cityId")%></span><input class="m-wrap" name="cityId" id="cityId" type="text"  						maxLength="10" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("districtCounty")%></span><input class="m-wrap" name="districtCounty" id="districtCounty" type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("districtCountyId")%></span><input class="m-wrap" name="districtCountyId" id="districtCountyId" type="text"  						maxLength="10" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("address")%></span><input class="m-wrap" name="address" id="address" type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("recipient")%></span><input class="m-wrap" name="recipient" id="recipient" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("recipientPhone")%></span><input class="m-wrap" name="recipientPhone" id="recipientPhone" type="text"  						maxLength="20" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/receiptaddress/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/receiptaddress/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: 250,
					colNames:['province','city',
...
,'<%=IReceiptAddressConstants.TABLE_COLUMN_DISPLAY.get("isDefault")%>'
					...
					colModel:[
					    ...
					    {name:'province',index:'province',width:90, editable:true,hidedlg:true,hidden:true},
					    {name:'city',index:'city',width:90, editable:true,hidedlg:true,hidden:true},
