  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("productId")%></span>						
							  <input type="text" class="m-wrap"  name="productId" />
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/price/insert',$(insertForm).serialize())">
       <i class="icon-ok"></i>
               保存</a>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<%--
							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("productId")%></span>						<input type="text" class="m-wrap"  name="productId" id="productId" />
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/product/reference?referenceInputType=radio','insertForm.productId,insertForm.productIdName');">选择</span>
							</div>
							 --%>

							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("productName")%></span><input class="m-wrap" name="productName" id="productName" type="text"  		readonly="readonly"				maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("price")%></span><input class="m-wrap" name="price" id="price" type="text"  						maxLength="19" />
							</div>
							<%--
							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("startTime")%></span><input name="startTime" id="startTime"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("startTime")%>" />
							</div>
							 
							<div class="input-prepend">
							  <span class="add-on"><%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("endTime")%></span><input name="endTime" id="endTime"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("endTime")%>" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/price/updateGengeral',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/product/reference?referenceInputType=radio','insertForm.productId,insertForm.productIdName');">选择</span>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/price/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/price/ajaxList?${queryString}',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("productName")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("price")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("status")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("startTime")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("endTime")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("create_user_name")%>'
,'<%=IPriceConstants.TABLE_COLUMN_DISPLAY.get("create_time")%>'
			...
			{name:'status',index:'status', width:90,editable: true,editoptions:{size:"20",maxLength:"1"},formatter:priceStatusConvert},
			...
			{name:'create_user_name',index:'price', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}},
			{name:'create_time',index:'endTime',width:90, editable:true, sorttype:"date",formatter:formatDateRow},
					], 
