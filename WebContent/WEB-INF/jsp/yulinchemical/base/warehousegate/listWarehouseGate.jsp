<%@page import="com.dfhc.base.warehousegate.IWarehouseGateConstants" %>
...
function insertSave(){
	submitInsertAjax('<%=request.getContextPath()%>/warehousegate/insert',$(insertForm).serialize());
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="100" />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%></span><%=RmJspHelper.getSelectField("isEnable", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("isEnable") + "'  ", true) %>							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>           	                       <textarea class="medium m-wrap" name="remark" id="remark" 					inputName="<%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 						 maxLength="300" 					 ></textarea>
							</div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name" id="name" type="text"  						maxLength="100" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/warehousegate/update',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/warehousegate/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/warehousegate/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%>'
,'<%=IWarehouseGateConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
],
					colModel:[
			...
			{name:'remark',index:'remark', width:90,editable: true,editoptions:{size:"20",maxLength:"300"}}
					...
					editurl: getRootPath()+'/warehousegate/update',//nothing is saved
					caption: "仓库门号列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/warehousegate/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
