   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("shipperCode")%></span><input type="text" class="text_field_reference text_field_readonly"  hiddenInputId="shipperCode" name="shipperCodeName" inputName="<%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("shipperCode")%>" value="" /><input type="hidden" name="shipperCode"><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onclick="javascript:getReference(new Array(form.shipperCode, form.shipperCodeName), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/shipper/reference?referenceInputType=radio');"/>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("targetId")%></span><input type="text" class="text_field_reference text_field_readonly"  hiddenInputId="targetId" name="targetIdName" inputName="<%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("targetId")%>" value="" /><input type="hidden" name="targetId"><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onclick="javascript:getReference(new Array(form.targetId, form.targetIdName), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/requisitionwarehouse/reference?referenceInputType=radio');"/>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("targetType")%></span><%=RmJspHelper.getSelectField("targetType", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_TARGET_TYPE), "", "inputName='" + IFreightConstants.TABLE_COLUMN_DISPLAY.get("targetType") + "'  ", true) %>							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/freight/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/freight/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: 250,
					colNames:['id',
...
,'<%=IFreightConstants.TABLE_COLUMN_DISPLAY.get("mileage")%>'
			...
			{name:'targetId',index:'targetId', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}},
			{name:'targetType',index:'targetType', width:90,editable: true,editoptions:{size:"20",maxLength:"2"}},
