<%@page import="com.dfhc.base.todomenumap.ITodoMenuMapConstants" %>
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuName")%></span><input class="m-wrap" name="menuName"  type="text"  						maxLength="200" />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuSql")%></span><input class="m-wrap" name="menuSql"  type="text"  />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuUrl")%></span><input class="m-wrap" name="menuUrl"  type="text"  						maxLength="1000" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/todomenumap/insert',$(insertForm).serialize())">
       <i class="icon-ok"></i>
               保存</a>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuName")%></span><input class="m-wrap" name="menuName" id="menuName" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuSql")%></span><input class="m-wrap" name="menuSql" id="menuSql" type="text"  />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuUrl")%></span><input class="m-wrap" name="menuUrl" id="menuUrl" type="text"  						maxLength="1000" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/todomenumap/update',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/todomenumap/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/todomenumap/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:[
'<%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuName")%>'
,'<%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuSql")%>'
,'<%=ITodoMenuMapConstants.TABLE_COLUMN_DISPLAY.get("menuUrl")%>'
],
					colModel:[
			{name:'menuName',index:'menuName', width:90,editable: true,editoptions:{size:"20",maxLength:"200"}},
			{name:'menuSql',index:'menuSql', width:90,editable: true,editoptions:{size:"20"}},
			{name:'menuUrl',index:'menuUrl', width:90,editable: true,editoptions:{size:"20",maxLength:"1000"}}
					], 
					...
					editurl: getRootPath()+'/todomenumap/update',//nothing is saved
					caption: "待办菜单映射表列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				function to_detail(){
				    	...
				    	window.location.href="${pageContext.request.contextPath}/todomenumap/toDetail?id="+id+"&pcode=${pcode}";
					}				
			         //新增弹出			     
				 function addShow(){			    	 
			    	 $(".ui_out_tan").show();
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/todomenumap/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
                      ...
                      $("#portlet-config-edit").find("input[id=menuName]").val(data.bean.menuName);
                      $("#portlet-config-edit").find("input[id=menuSql]").val(data.bean.menuSql);
                      $("#portlet-config-edit").find("input[id=menuUrl]").val(data.bean.menuUrl);
                	   	   }
                       },
