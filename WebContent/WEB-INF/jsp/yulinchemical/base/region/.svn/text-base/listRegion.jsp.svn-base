<%@page import="com.dfhc.base.region.IRegionConstants" %>
...
function _saveInsert(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/region/insert',$(insertForm).serialize());
...
function _saveUpdate(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/region/update',$(updateForm).serialize());
}
</script>
</head>
<body>
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullX" name="name"  type="text"  						maxLength="200" />
							<i class="notNull">*</i>
							</div>
							
							<div class="input-prepend" style=" height: 45px" >
							  <span class="add-on"><%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>           	                       
							  <textarea cols="3" rows="5" class="medium m-wrap" name="remark" id="remark" inputName="<%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 	 maxLength="1000"  ></textarea>
							</div>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="200" />
							</div>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/region/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/region/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IRegionConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
],
					colModel:[
			...
			{name:'remark',index:'remark', width:90,editable: true,editoptions:{size:"20",maxLength:"300"}}
					...
					editurl: getRootPath()+'/region/update',//nothing is saved
					caption: "区域维护列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/region/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
