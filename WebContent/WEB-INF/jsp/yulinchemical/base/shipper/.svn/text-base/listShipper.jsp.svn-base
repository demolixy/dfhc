<%@page import="com.dfhc.base.shipper.IShipperConstants" %>
...
// 	var rowData = $("#grid-table").jqGrid('getRowData',selectedId);
	...
	var url='${pageContext.request.contextPath}/shipper/ajaxDeleteShipper';
	$.ajax({
	    type:'post',//可选get
	    url:url,
...
function _saveInsert(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/shipper/insert',$(insertForm).serialize());
...
function _saveUpdate(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/shipper/update',$(updateForm).serialize());
}
$(function(){
    orderStatusShow=[]; 
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullX" name="name"  type="text"  						maxLength="300" />
							<i class="notNull">*</i>
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("code")%></span>
							  <input class="m-wrap notNullX" name="code"  type="text"  						maxLength="10" />
							<i class="notNull">*</i>
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("isDefault")%></span><%=RmJspHelper.getSelectField("isDefault", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IShipperConstants.TABLE_COLUMN_DISPLAY.get("isDefault") + "'  ", true) %>							
							  </div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullY" name="name" id="name" type="text"  						maxLength="300" />
							<i class="notNull">*</i>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("code")%></span>
							  <input class="m-wrap notNullY" name="code" id="code" type="text"  						maxLength="10" />
							<i class="notNull">*</i>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("isDefault")%></span><%=RmJspHelper.getSelectField("isDefault", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IShipperConstants.TABLE_COLUMN_DISPLAY.get("isDefault") + "'  ", true) %>							</div>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("code")%></span><input class="m-wrap" name="code"  type="text"  						maxLength="10" />
							</div>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/shipper/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/shipper/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("code")%>'
,'<%=IShipperConstants.TABLE_COLUMN_DISPLAY.get("isDefault")%>'
],
					colModel:[
			{name:'name',index:'name', width:90,editable: true,editoptions:{size:"20",maxLength:"300"}},
			{name:'code',index:'code', width:90,editable: true,editoptions:{size:"20",maxLength:"10"}},
			{name:'isDefault',index:'isDefault', width:90,editable: true,editoptions:{size:"20",maxLength:"1"},formatter:orderStatus}
					...
					editurl: getRootPath()+'/shipper/update',//nothing is saved
					caption: "发货方维护列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/shipper/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
                        async:false , //false为同步
                        success: function(data,status) {
                	   	   if(data.status=="0"){
                      ...
                      $("#portlet-config-edit").find("input[id=code]").val(data.bean.code);
			     ...
			     function deleteShow (){
			    	 ...
			    	 var checkedLen= $(grid_selector).jqGrid("getGridParam","selarrrow");
				    	if(checkedLen==0){
