<%@page import="com.dfhc.base.paymentmode.IPaymentModeConstants" %>
...
function _saveInsert(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/paymentmode/insert',$(insertForm).serialize());
...
function _saveUpdate(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/paymentmode/update',$(updateForm).serialize());
}
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullA" name="name"  type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>

							
							<div class="input-prepend" style="    height: 45px;">
							  <span class="add-on"><%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%></span><%=RmJspHelper.getSelectField("isEnable", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("isEnable") + "'  ", true) %>							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>           	                       <textarea class="medium m-wrap" name="remark" id="remark" 					inputName="<%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 						 maxLength="300" 					 ></textarea>
							</div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullE" name="name" id="name" type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap " name="name"  type="text"  						maxLength="100" />
							</div>
							<%--
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/paymentmode/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/paymentmode/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%>'
,'<%=IPaymentModeConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
],
					colModel:[
			...
			{name:'remark',index:'remark', width:90,editable: true,editoptions:{size:"20",maxLength:"300"}}
					...
					editurl: getRootPath()+'/paymentmode/update',//nothing is saved
					caption: "收款方式列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/paymentmode/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
