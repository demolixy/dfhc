<%@page import="com.dfhc.base.warehouseposition.IWarehousePositionConstants" %>
...
function insertSave(){
	...
	submitInsertAjax('<%=request.getContextPath()%>/warehouseposition/insert',$(insertForm).serialize());
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("warehouseGateId")%></span>						<input type="text" class="m-wrap"  name="warehouseGateId" />
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/warehousegate/reference?referenceInputType=radio','insertForm.warehouseGateId,insertForm.warehouseGateIdName');">选择</span>
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="100" />
							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%></span><%=RmJspHelper.getSelectField("isEnable", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("isEnable") + "'  ", true) %>							</div>

							
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>           	                       <textarea class="medium m-wrap" name="remark" id="remark" 					inputName="<%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 						 maxLength="300" 					 ></textarea>
							</div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("warehouseGateId")%></span>						<input type="text" class="m-wrap"  name="warehouseGateId" id="warehouseGateId" />
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/warehousegate/reference?referenceInputType=radio','insertForm.warehouseGateId,insertForm.warehouseGateIdName');">选择</span>
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name" id="name" type="text"  						maxLength="100" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitInsertAjax('<%=request.getContextPath()%>/warehouseposition/update',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/warehousegate/reference?referenceInputType=radio','insertForm.warehouseGateId,insertForm.warehouseGateIdName');">选择</span>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/warehouseposition/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/warehouseposition/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("warehouseGateId")%>'
,'<%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("name")%>'
,'<%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("isEnable")%>'
,'<%=IWarehousePositionConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
],
					colModel:[
			{name:'warehouseGateId',index:'warehouseGateId', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}},
			{name:'name',index:'name', width:90,editable: true,editoptions:{size:"20",maxLength:"100"}},
			{name:'isEnable',index:'isEnable', width:90,editable: true,editoptions:{size:"20",maxLength:"1"}},
			{name:'remark',index:'remark', width:90,editable: true,editoptions:{size:"20",maxLength:"300"}}
					...
					editurl: getRootPath()+'/warehouseposition/update',//nothing is saved
					caption: "仓位号列表展示",
					autowidth: true
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/warehouseposition/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
                      ...
                      $("#portlet-config-edit").find("input[id=warehouseGateId]").val(data.bean.warehouseGateId);
                      $("#portlet-config-edit").find("input[id=name]").val(data.bean.name);
                      $("#portlet-config-edit").find("input[id=isEnable]").val(data.bean.isEnable);
