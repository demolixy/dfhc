<%--
   ...
   *
   * 文件名称：referenceTrainStation.jsp
   *
   * 功能描述：火车站点参照页面
   * 
   * 版本历史：
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ITrainStationConstants.TABLE_COLUMN_DISPLAY.get("stationName")%></span><input class="m-wrap" name="stationName" id="stationName" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=ITrainStationConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>					<textarea class="medium m-wrap" name="remark" id="remark" 					inputName="<%=ITrainStationConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 						 maxLength="300" 					 ></textarea>
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/trainstation/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/trainstation/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:['stationName',
'<%=ITrainStationConstants.TABLE_COLUMN_DISPLAY.get("stationName")%>'
,'<%=ITrainStationConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
],
					colModel:[
					    {name:'stationName',index:'stationName',width:90, editable:true,hidedlg:true,hidden:true},		
