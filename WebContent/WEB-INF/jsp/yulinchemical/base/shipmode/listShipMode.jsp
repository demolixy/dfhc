...
function _saveInsert(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/shipmode/insert',$(insertForm).serialize());
...
function _saveUpdate(){	
	...
	submitInsertAjax('<%=request.getContextPath()%>/shipmode/update',$(updateForm).serialize());
}
$(function(){
    orderStatusShow=[]; 
  ...
  <div class="ui-jqdialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix"  style="cursor: move;">
   ...
   <div class="control-group">
			 <div class="controls">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullX" name="name"  type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("code")%></span>
							  <input class="m-wrap notNullX" name="code"  type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("isTray")%></span><%=RmJspHelper.getSelectField("isTray", -1, RmGlobalReference.get(ISystemConstant.DICTIONARY_RM_YES_NOT), "", "inputName='" + IShipModeConstants.TABLE_COLUMN_DISPLAY.get("isTray") + "'  ", true) %>							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("remark")%></span>           	                       
							  <textarea cols="3" rows="5" class="medium m-wrap" name="remark" id="remark" inputName="<%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("remark")%>" 	 maxLength="1000"  ></textarea>
							</div>
   ...
   <form name="updateForm" id="updateForm" class="FormGrid ui_inner_form" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span>
							  <input class="m-wrap notNullY" name="name" id="name" type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("code")%></span>
							  <input class="m-wrap notNullY" name="code" id="code" type="text"  						maxLength="100" />
							<i class="notNull">*</i>
							</div>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("name")%></span><input class="m-wrap" name="name"  type="text"  						maxLength="100" />
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("code")%></span><input class="m-wrap" name="code"  type="text"  						maxLength="100" />
							</div>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/shipmode/logicDelete?pcode=${pcode }');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/shipmode/ajaxList',
					datatype: "json",
				    mtype: 'POST',  
					height: "100%",
					colNames:[
'<%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("id")%>'
...
,'<%=IShipModeConstants.TABLE_COLUMN_DISPLAY.get("isTray")%>'
					...
					colModel:[
			{name:'id',index:'id', width:90,editable: true,hidedlg:true,hidden:true,editoptions:{size:"20",maxLength:"100"}},
			   ...
			   jQuery(grid_selector) .navButtonAdd(pager_selector,
				...
				//查询
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/shipmode/get/" + updateDate+"/"+new Date(),
                        dataType: "json",
                      ...
                      $("#portlet-config-edit").find("select[name=isEnable]").val(data.bean.isEnable);
                      $("#portlet-config-edit").find("textarea[id=remark]").val(data.bean.remark);
                      $("#portlet-config-edit").find("select[name=isTray]").val(data.bean.isTray);
                	   	   }
                       },
