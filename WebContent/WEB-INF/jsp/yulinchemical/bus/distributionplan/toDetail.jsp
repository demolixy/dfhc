...
function _delete(urlData){
      ...
      deleteForm.action=urlData+"&ids="+ids+"&id=${bean.id}";
      deleteForm.submit();		
...
//insert
function   insertSubmit(url,param){
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/customer/reference?referenceInputType=radio&isMainProduct=1','insertForm.customer_id,insertForm.customerName,insertForm.customerId');">选择</span><span class="style_required_red">* </span>
							</div>
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/product/reference?referenceInputType=radio&main_secondary_product_flag=01','insertForm.productId,insertForm.goodsName');">选择</span><span class="style_required_red">* </span>
							...
							<div class="input-prepend" style="margin-top:-11px;">
								...
								<%=RmJspHelper.getSelectField("shipperCode",-1, RmProjectHelper.getCommonServiceInstance().paseToArrays("select CODE as data_key,name as data_value from BASE_SHIPPER where nvl(BASE_SHIPPER.DELETE_FLAG,'0') = 0"),"01", "    inputName=\""+ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("shipperCode")+"\" validate='notNull;'  ", true , "--请选择--") %>			
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("sendCarDate")%></span><input name="sendCarDate"  class="m-wrap " readonly size="16" type="text" value="" inputName="<%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("sendCarDate")%>" />
							</div>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("region")%></span><input class="m-wrap" name="region"  type="text"  						maxLength="2" />
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("area")%></span><input class="m-wrap" name="area"  type="text"  						maxLength="2" />
							</div>
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/flowto/reference?referenceInputType=radio','insertForm.flowToId,insertForm.flowToIdName');">选择</span><span class="style_required_red">* </span>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:insertSubmit('<%=request.getContextPath()%>/cardistributionplan/insert',$(insertForm).serialize())">
       <i class="icon-ok"></i>
               保存</a>
...
<%--
	 ...
	 --%>						
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("region")%></span><input class="m-wrap" name="region" id="region" type="text"  	maxLength="2" />
							</div>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("area")%></span><input class="m-wrap" name="area" id="area" type="text"  						maxLength="2" />
							</div>
...
--%>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("status")%></span><input class="m-wrap" name="status" id="status" type="text"  		readonly="readonly"/>				</div>
...
--%>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:submitUpdate('<%=request.getContextPath()%>/cardistributionplan/update',$(updateForm).serialize())">
       <i class="icon-ok"></i>
            更新</a>
       <a href="javascript:void(0)" id="cData" class="cancel fm-button ui-state-default ui-corner-all fm-button-icon-left">
      <i class="icon-remove"></i> 
...
--%>
   ...
   <form name="deleteForm" id="deleteForm" class="FormGrid ui_inner_form"  method="post"  >
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left btn-danger" onclick="javascript:_delete('<%=request.getContextPath()%>/cardistributionplan/doDelete?pcode=${pcode}');">
		                       ...
		                       <div class="widget-main">
		                         ...
		                         <table  class="table table-striped table-bordered table-hover detail-tab">
			                         ...
			                         <tr>
			                             ...
			                             <th><%=IDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("planCode")%>:</th>
			                             <td colspan="2"><c:out value="${bean.planCode}"></c:out>   </td>
			                             <th><%=IDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("planDistributionDate")%>:</th>
			                             <td colspan="2"><c:out value="${bean.planDistributionDate}"></c:out> </td>
			                          
			                          </tr>
			                         <tr>
		                           
			                             <th><%=IDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("remark")%>:</th>
			                             <td colspan="5"><c:out value="${bean.remark}"></c:out>   </td>
			                        </tr>
			                     <c:if test="${not empty bean.sendCarPlanImage }">
			 <%--                        <tr>
		                           
			                             <th><input  type="button" value="查看图片" onclick="window.open('<%=request.getContextPath()%>/distributionplan/showImg?id=${bean.id}')" /></th>
			                        </tr>
			      --%>                   
								</c:if>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/cardistributionplan/ajaxList?distributionPlanId=${bean.id}',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:[
...
,'<%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'logisticsCompany',index:'logisticsCompany', width:90,editable: true,editoptions:{size:"20",maxLength:"200"}},
			...
			{name:'plannedMileage',index:'plannedMileage', width:90,editable: true,editoptions:{size:"20",maxLength:"10"}},
...
// 				      position:"last" }) ;
				...
				//查询
				 ...
				 function addShow(){
                     ...
                     $("#insertForm").find("input[name=planCode]").val('${bean.planCode}');
                     $("#insertForm").find("input[name=distributionPlanId]").val('${bean.id}');
                     $("#insertForm").find("input[name=sendCarDate]").val('${bean.planDistributionDate}');
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                      ...
                      $("#portlet-config-edit").find("input[name=status]").val(orderStatusShow[data.bean.status]);
