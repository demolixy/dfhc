...
function updateLadingBill(flag){
	$.ajax({
        type: "POST",
        url: "<%=request.getContextPath()%>/ladingbill/invalidLadingBill?date=" + new Date(),
        data:{ "ladingBillId":currentLadingBillId, "invalidReason":$("textarea[name=invalidReason]").val()},
        dataType: "json",
        ...
        success: function(data,status) {
	   	 ...
	   	 $("#portlet-config-delete").hide(); 
	   		   ...
	   		   alert(data.message);
	   		   window.location = "<%=request.getContextPath()%>/ladingbill?operation=ladingBillManage";
	   	   }
       },
		                       ...
		                       <div class="widget-main">
			                    ...
			                    <tr>
			                          ...
			                          <tr>
			                             ...
			                             <th>货物价格:</th>
			                             <td><c:out value="${bean.price}"></c:out>  </td>
			                          </tr>

			                          <tr>
			                             <th>实际里程:</th>
			                             <td><c:out value="${bean.factMileage}"></c:out>  </td>
			                             <th>计价里程:</th>
			                             <td><c:out value="${bean.valuationMileage}"></c:out>  </td>
			                             <th>结算运费(元/吨):</th>
			                             <td><c:out value="${bean.settlementFreight}"></c:out>  </td>
			                          </tr>

			                          <tr>
			                             <th>总运费(元):</th>
			                             <td><c:out value="${bean.totalFreight}"></c:out>  </td>
			                             <c:choose>
				                         	<c:when test="${ladingBillStatus == '01' || ladingBillStatus == '00'}">
												<th>作废原因:</th>
			                             		<td colspan="3">
			                             			<textarea rows="" cols="90" name="invalidReason"></textarea>	
			                             		</td>				                         	
				                         	</c:when>
				                         </c:choose>
			                          </tr>
		                   ...
		                   <div class="row">
							<div class="col-xs-12">
								...
								<!-- PAGE CONTENT ENDS -->
		                         ...
		                         <c:choose>
		                         	<c:when test="${ladingBillStatus == '01' || ladingBillStatus == '00'}">
				                          <a href="javascript:void(0);" class="sub-btn-shi"  id="commit"   onclick="javascript:doSaveOrCommit('commit')" >作废</a>
...
// 				      position:"last" }) ;
			...
			function doSaveOrCommit(flag){
				...
				$.ajax({
				    type:'post',//可选get
				    url:"${pageContext.request.contextPath}/cardistributionplan/doZhuToLadingBillList?operation=${operation}&flag=" + flag + "&date=" + new Date(),
				    ...
				    success:function(data,status){
					  if(data.status=="0"){
						  ...
						  alert(data.message);
				   	     	...
				   	     	alert(data.message);
// 		                    window.location.href="${pageContext.request.contextPath}/cardistributionplan/listCarDistributionLading";
				   	    	 window.location = '${uri}';
				   	   }
				   },
				   error:function(xhr,s1,s2){
