						...
						<ul class="breadcrumb">
							...
							<c:choose>
								<c:when test="${operation == 'coordination' }">
									...
									<li class="active">汽运销售计划车辆安排</li>
								</c:when>
								<c:otherwise>
									<li>
										<a href="#">提货单管理</a>
									</li>
									<li class="active">汽配运送提货单</li>
								</c:otherwise>
							</c:choose>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/cardistributionplan/ajaxList?businessType=00&status=02&operation=${operation}&operationLu=operationLu',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:[
...
,'<%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'makeDate',index:'makeDate',width:130,sorttype :"date",formatter: formatDateShow},
			...
			{name:'sendCarDate',index:'sendCarDate',width:130, editable:true, sorttype:"date",formatter: formatDateShow},
			{name:'receivingPerson',index:'receivingPerson', width:130,editable: true,editoptions:{size:"20",maxLength:"200"}},
			{name:'receivingPersonPhone',index:'receivingPersonPhone', width:130,editable: true,editoptions:{size:"20",maxLength:"20"}},
...
// 					   window.location.href="${pageContext.request.contextPath}/cardistributionplan/toAddZhuProductLift?pcode=${pcode}";
			     ...
			     function updateShow(){
                    ...
                    var updateDate=$(pointer).attr("id");
  				   window.location.href="${pageContext.request.contextPath}/cardistributionplan/toUpdateCarDistributionLading?operation=${operation}&id="+updateDate+"&pcode=${pcode}";
