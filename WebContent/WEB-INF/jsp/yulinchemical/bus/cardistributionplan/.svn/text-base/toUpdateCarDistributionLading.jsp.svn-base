...
// 			    	 }
			    	 ...
			    	 $("#insertForm").find("input[name=nuclearLoad]").val("${bean.num}");
	                        ...
	                        success: function(data,status) {
	                	   	   if(data.status=="0"){
	                	   	       alert(data.message);
	                    	   		...
	                    	   		alert(data.message);
	               				   window.location.href="${pageContext.request.contextPath}/cardistributionplan/toUpdateCarDistributionLading?operation=${operation}&id=${bean.id}&pcode=${pcode}";
	                	   	   }
	                       },
	                       error:function(xhr,s1,s2){
	                       	var message = xhr.responseText;
				    ...
				    success:function(data,status){
					  if(data.status=="0"){
						  ...
						  alert(data.message);
				   	     	...
				   	     	alert(data.message);
		                    window.location.href="${pageContext.request.contextPath}/cardistributionplan/listCarDistributionLading?operation=${operation}";
				   	   }
				   },
				   error:function(xhr,s1,s2){
				    	var message = xhr.responseText;
