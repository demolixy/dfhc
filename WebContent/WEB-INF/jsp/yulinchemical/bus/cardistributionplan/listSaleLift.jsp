...
--%>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=ICarDistributionPlanConstants.TABLE_COLUMN_DISPLAY.get("status")%></span><input class="m-wrap" name="statusShow" id="statusShow" type="hidden"  		readonly="readonly"/>	<input class="m-wrap" name="status" id="status" type="text"  		readonly="readonly"/>				</div>
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/cardistributionplan/ajaxList?businessType=01',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:[
			  ...
			  jQuery(grid_selector) .navButtonAdd(pager_selector,
				{ caption:"",title:"提交", buttonicon:"icon-group red",
			     ...
			     function updateShow(){
                    ...
                    if(rowData.status != orderStatusShow[<%=ISystemConstant.DICTIONARY_PLAN_STATUS_11%>]){
                    	alert("该计划单不能修改！");
                    	return false;
                    }
                    $.ajax({
                      ...
                      $("#portlet-config-edit").find("input[name=logisticsCompany]").val(data.bean.logisticsCompany);
                      $("#portlet-config-edit").find("input[name=logisticsCompanyId]").val(data.bean.logisticsCompanyId);
                      ...
                      $("#portlet-config-edit").find("input[name=goodsName]").val(data.bean.goodsName);
                      $("#portlet-config-edit").find("input[name=productId]").val(data.bean.productId);
                      ...
                      $("#portlet-config-edit").find("input[name=status]").val(data.bean.status);
                      ...
                      $("#portlet-config-edit").find("input[name=planCarNum]").val(data.bean.planCarNum);
                	   	   }
                       },
				...
				function  formatDateShow(cellvalue, options, cell){
			     ...
			     function to_commit(){
	    	        ...
	    	        $.ajax({
	    	            type:'post',//可选get
	    	            url:'<%=request.getContextPath()%>/cardistributionplan/doSaleCommit',
