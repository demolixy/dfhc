...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("signTime")%>'
...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'signTime',index:'signTime', width:90,editable: true,editoptions:{size:"20",maxLength:"20"}},
			
			{name:'takeNoTime',index:'takeNoTime',width:90, editable:true, sorttype:"date", formatter:formatDateRow}
,
...
,
			...
			{name:'remark',index:'remark', width:90,editable: true,editoptions:{size:"20",maxLength:"200"}},
				...
				//查看
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"审核", buttonicon:"icon-eye-open red",
				      onClickButton:showDetail , 
				      position:"last" }) ;
				//查看
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"批量审核", buttonicon:"icon-check red",
				      onClickButton:batchAudit , 
				      position:"last" }) ;
			     ...
			     function batchAudit(){
			    	 var checkedLen= $(".cbox:checked").length;
				    	...
				    	for (var g = 0; g < allcheck.length; g++) {
				    		...
				    		if ($(allcheck[g]).is(":checked")) {
				    			ids += $(allcheck[g]).parents("tr").attr("id");
				    	...
				    	if(confirm("是否确认批量审核？")){
	                    	$.ajax({
	                            type: "POST",
	                            url: "<%=request.getContextPath()%>/ladingbill/batchAuditLadingBill?date="+new Date(),
	                            data:{"ladingBillIds":ids},
	                            dataType: "json",
			     ...
			     function updateShow(){
                    ...
                    $.ajax({
                      ...
                      $("#portlet-config-edit").find("input[id=signTime]").val(data.bean.signTime);
                	   	   }
                       },
                       error:function(xhr,s1,s2){
                       	var message = xhr.responseText;
				...
				function showDetail(){
	                ...
	                window.location = "${pageContext.request.contextPath}/ladingbill/ladingBillDetailForBillType?ladingBillId=" + updateDate + "&flag=01&uri=" + window.location.pathname + window.location.search + "&date=" + new Date();
