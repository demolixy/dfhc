...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/receiptconfletter/reference?referenceInputType=radio','insertForm.invoiceSettlementId,insertForm.invoiceSettlementIdName');">选择</span>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on">客户名称</span> <input type="text" class="tab-inp" name="customerName"/> <input type="hidden" class="tab-inp" name="customerId"/>
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/customer/referenceCustomerFinance?referenceInputType=radio&isByProduct=1&operation=customerFinance','form.customer_id,form.customerName,form.customerId,form.amount,form.amount2');">选择</span>
			               </div>
...
,'发货日期','客户名称','产品名称','单价','实际发货数量','实际发货金额','车牌号码','司机姓名','联系方式'
],
					colModel:[
			...
			{name:'actualShippingAmount',index:'actualShippingAmount', width:90,editable: true},
