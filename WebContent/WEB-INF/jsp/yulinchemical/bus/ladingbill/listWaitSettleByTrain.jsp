...
<%--
 ...
 --%>
							...
							<div class="input-prepend">
							  <span class="add-on">物流公司</span><input class="m-wrap" name="logisticsCompany" id="logisticsCompany" type="text"  						maxLength="200" />
							</div>
...
--%>
							<div class="input-prepend">
							  <span class="add-on">发货日期从</span><input name="outTimeStart" id="outTimeStart"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value=""  />
							</div>
							<div class="input-prepend">
							  <span class="add-on">到</span><input name="outTimeEnd" id="outTimeEnd"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value=""  />
							</div>
							<div class="input-prepend">
							  <span class="add-on"><%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("licensePlate")%></span><input class="m-wrap" name="licensePlate" id="licensePlate" type="text"  						maxLength="100" />
							</div>
				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">
							<c:if test="${billTypeLu=='02' }">汽配运费待结算</c:if>
							<c:if test="${billTypeLu=='03' }">汽调运费待结算</c:if>
							<c:if test="${billTypeLu=='04' }">铁调运费待结算</c:if>
							</li>
						</ul><!-- .breadcrumb -->
...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'outTime',index:'outTime',width:90, editable:true, sorttype:"date", formatter:formatDateRow},		
...
// 					//修改
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"多条修改", buttonicon:"icon-pencil blue",
...
// 				      position:"last" }) ;
				...
				//审核
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"生成结算单", buttonicon:"icon-group red",
					  onClickButton:doAdit , 
				      position:"last" }) ;
...
// 			     //新增弹出
			     ...
			     function doAdit(){
			    	 ...
			    	 $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/logisticscostsettlement/doSettle?ladingBillIds="+ids+"&billTypeLu=${billTypeLu}&pcode=${pcode}",
                        dataType: "json",
...
//                   $("#portlet-config-edit").show();
				...
				function updateList(){
			    	...
			    	if(confirm("确认修改吗")){
				   		 $.ajax({
							    type:'post',//可选get
							    url:"${pageContext.request.contextPath}/ladingbill/updateFactMileages",
							    data:{'ladingbillIds':ids,'factMileages':factMileages,'factValuationMileages':factValuationMileages,'yuanKmTons':yuanKmTons,'settlementFreights':settlementFreights},
							    dataType:'json',
							    async:false , //false为同步
							    success:function(data,status){
								  if(data.status=="0"){
									  ...
									  alert(data.message);
							   	     	...
							   	     	alert(data.message);
					                    window.location.href="${pageContext.request.contextPath}/ladingbill/listWaitSettleByTrain?listWaitSettleLu=listWaitSettleLu&billType=${billTypeLu}&pcode=${pcode}";
							   	   }
							   },
							   error:function(xhr,s1,s2){
							    	var message = xhr.responseText;
