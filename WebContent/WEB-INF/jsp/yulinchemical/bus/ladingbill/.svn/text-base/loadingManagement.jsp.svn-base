...
<!-- 								</span> -->
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				var statuss = "'<%=ISystemConstant.DICTIONARY_LADING_BILL_STATUS_03%>', '<%=ISystemConstant.DICTIONARY_LADING_BILL_STATUS_05%>'";
...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
					...
					colModel:[
								...
								{name:'billType',index:'billType', width:90,editable: true,editoptions:{size:"20",maxLength:"100"}},
...
// 				      position:"last" }) ;
				...
				//提货失败
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"提货失败", buttonicon:"icon-group red",
					  onClickButton:to_FailPick , 
				      position:"last" }) ;
// 				//开具装车单
// 				  jQuery(grid_selector) .navButtonAdd(pager_selector,
// 					{ caption:"",title:"开具装车单", buttonicon:"icon-eye-open red",
// 				      onClickButton:showDetail , 
// 				      position:"last" }) ;
				
				//查看
				  jQuery(grid_selector) .navButtonAdd(pager_selector,
					{ caption:"",title:"查看详细", buttonicon:"icon-eye-open red",
				      onClickButton:showDetail , 
				      position:"last" }) ;

				     //提货失败
				     function to_FailPick(){
			    	      var len = $(".cbox:checked").length;
				    	if(len==0){
				    		alert("请至少选择一条数据！");
				    		return false;
		    	        ...
		    	        $.ajax({
		    	            type:'post',//可选get
		    	            url:'<%=request.getContextPath()%>/ladingbill/doListFailPick',
				...
				function showDetail(){
                    ...
                    var pointer=$(".cbox:checked").parents("tr");
                    var updateDate=$(pointer).attr("id");
                    window.location = "${pageContext.request.contextPath}/ladingbill/ladingBillDetail?ladingBillId=" + updateDate + "&loadingNotice=loadingNoticeLu&date=" + new Date();
				}
			});
