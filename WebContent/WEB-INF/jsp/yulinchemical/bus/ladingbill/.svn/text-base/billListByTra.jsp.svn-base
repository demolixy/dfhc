...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/receiptconfletter/reference?referenceInputType=radio','insertForm.invoiceSettlementId,insertForm.invoiceSettlementIdName');">选择</span>
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
						...
						<div class="input-prepend">
							  <span class="add-on">发货开始日期</span><input name="outTimeStr" id="outTimeStr"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("outTime")%>" />
						</div> 
						<div class="input-prepend">
							  <span class="add-on">发货结束日期</span><input name="outTimeEnd" id="outTimeEnd"  class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("outTime")%>" />
						</div> 

							<div class="input-prepend">
							  <span class="add-on">站点</span><input class="m-wrap"  readonly="readonly"  name="arriveAtStation"  type="text"  />  <input class="m-wrap" name="stationId"  type="hidden"  />
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/trainstation/reference?referenceInputType=radio','insertForm.stationId,insertForm.arriveAtStation');">选择</span>
							...
							<div class="input-prepend">
							  <span class="add-on">身份证号</span><input class="m-wrap" name="driverIdCardNumber" id="driverIdCardNumber" type="text"  						maxLength="20" />
							</div>
							
							<div class="input-prepend">
							  <span class="add-on">运输公司</span><input class="m-wrap" name="logisticsCompany" id="logisticsCompany" type="text"  						maxLength="20" />
							</div>
...
,'发货日期','运输公司','产品名称','收货地址','车牌号码','实际发货数量','实际里程','结算运价','运费','司机姓名','联系方式','身份证'
],
					colModel:[
			...
			{name:'licensePlate',index:'licensePlate', width:90,editable: true},
			...
			{name:'freight',index:'freight', width:90,editable: true},
			{name:'driverName',index:'driverName', width:90,editable: true},
			{name:'phoneNumber',index:'phoneNumber', width:90,editable: true},
			{name:'driverIdCardNumber',index:'driverIdCardNumber', width:90,editable: true}
					], 
