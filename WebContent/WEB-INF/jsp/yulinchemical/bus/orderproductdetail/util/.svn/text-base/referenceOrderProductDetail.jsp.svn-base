<%--
   ...
   *
   * 文件名称：referenceOrderProductDetail.jsp
   *
   * 功能描述：销售订单产品明细参照页面
   * 
   * 版本历史：
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("orderId")%></span><input type="text" class="text_field_reference text_field_readonly"  hiddenInputId="orderId" name="orderIdName" inputName="<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("orderId")%>" value="" /><input type="hidden" name="orderId"><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onclick="javascript:getReference(new Array(form.orderId, form.orderIdName), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/order/reference?referenceInputType=radio');"/>
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/orderproductdetail/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/orderproductdetail/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: 250,
					colNames:['id',
'<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("orderId")%>'
,'<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("orderNum")%>'
,'<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("productTypeId")%>'
,'<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("productId")%>'
...
,'<%=IOrderProductDetailConstants.TABLE_COLUMN_DISPLAY.get("remainderCarNum")%>'
			...
			{name:'orderId',index:'orderId', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}},
			{name:'orderNum',index:'orderNum', width:90,editable: true,editoptions:{size:"20",maxLength:"13"}},
			{name:'productTypeId',index:'productTypeId', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}},
