...
$(function(){
	...
	<c:otherwise>
	 $.ajax({
		    type:'post',//可选get
		    url:'${pageContext.request.contextPath}/customer/ajaxList?isByProduct=1&operation=customerFinance&userId=${loginUserVo.id}',
		    dataType:'json',
		    ...
		    success:function(data,status){
			  if(data.status=="0"){
				  alert(data.message);
		   		   ...
		   		   debugger;
		   		if(typeof(data.rows)!='undefind'){
		   			for(var i=0;i<data.rows.length;i++){
		   				$("#zongForm").find("input[name=customerName]").val(data.rows[i].name);
		   				$("#zongForm").find("input[name=customerId]").val(data.rows[i].userId);
		   				$("#zongForm").find("input[name=amount]").val(data.rows[i].amount);
		   				$("#zongForm").find("input[name=amount2]").val(data.rows[i].amount2);
		   			}
		   		}
		   	   }
		   },
...
function changePlannedVolume(obj){
	...
	$(obj).parents("form").find("input[name='remarkVolume']").val(plannedVolume);
	
	//自动计算车数量
	var carNum=Math.ceil(plannedVolume/25);
	$(obj).parents("form").find("input[name='carNum']").val(carNum);
	$(obj).parents("form").find("input[name='remainderCarNum']").val(carNum);
...
function productSelected(){
	...
	var customerId = $("form[name=zongForm]").find("input[name=customerId]").val();
	...
	getNewReference('<%=request.getContextPath()%>/product/reference?referenceInputType=radio&main_secondary_product_flag=02&operation=productPrice','insertForm.productId,insertForm.productName,insertForm.productTypeId,insertForm.unitPrice');
}
...
<span class="choose_ref"  onclick="javascript:productSelected();">选择</span>
							</div>
 ...
 <!-- begin  详情 -->
		         ...
		         <%--               <a href="javascript:void(0)" title="审核通过">
		          ...
		          <form name="zongForm" id="zongForm"  method="post"  >
		                ...
		                <c:choose>
						  <c:when test="${empty operation }">
			                             <td colspan="2"><input type="text" class="tab-inp" name="customerName" readonly="readonly"/> <input type="hidden" class="tab-inp" name="customerId"/>
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/customer/referenceCustomerFinance?referenceInputType=radio&isByProduct=1&operation=customerFinance','zongForm.customer_id,zongForm.customerName,zongForm.customerId,zongForm.amount,zongForm.amount2');">选择</span>
			                              </td>
		                  </c:when>
		                  <c:otherwise>
			                             <td colspan="2"><input type="text" class="tab-inp" name="customerName" value="" readonly="readonly"/> <input type="hidden" class="tab-inp" name="customerId"  value=""/>
			                             </td>
		                  </c:otherwise>         
			              </c:choose>           
			                             ...
			                             <td colspan="2"><input type="text" class="tab-inp" value="汽运自提" name="shipMode"  readonly="readonly"/> <input type="hidden" class="tab-inp" name="shipModeId"  value="00"/> 
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/shipmode/reference?referenceInputType=radio','zongForm.shipModeId,zongForm.shipMode');">选择</span>
			                          ...
			                          <tr>
			                             ...
			                             <td colspan="2"><input type="text"  class="tab-inp date-picker" readonly name="deliveryStartDate" />   </td>
 			                             <th>合同金额:</th>
			                         	<td>
			                          ...
			                          <tr>
			                          	<th>业务余额:</th>
			                          	<td  colspan="2">
			                          		<input type="text" class="tab-inp"  readonly="readonly"  name="amount2"/>
			                          	</td>
			                          	<th>剩余业务余额:</th>
			                             <td >
