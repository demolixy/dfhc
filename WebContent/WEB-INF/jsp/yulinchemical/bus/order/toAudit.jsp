...
//审核
function  doAudit(isAudit){
	$("#audit").attr("disabled","disabled");
	$("#noAudit").attr("disabled","disabled");
	var url;
	if(isAudit=='audit'){
		if(!confirm("本次订单合同金额 ${price.PRICE}，剩余业务金额 ${price.AVAILABLECREDIT}")){
			return  false;
		}
		url="${pageContext.request.contextPath}/order/doAudit";
	}else if(isAudit=='noAudit'){
		if(!confirm("确定审核驳回吗")){
			return  false;
		}
		url="${pageContext.request.contextPath}/order/doNoAudit";
	}
	 $.ajax({
		    type:'post',//可选get
		    url:url,
		    ...
		    success:function(data,status){
			  if(data.status=="0"){
       			 ...
       			 $("#noAudit").removeAttr("disabled");
				  alert(data.message);
		   	   }else{
		   	     	alert(data.message);
                    window.location.href="${pageContext.request.contextPath}/order/listAudit?pcode=${pcode}";
		   	   }
		   },
...
<span class="choose_ref"  onclick="javascript:getNewReference('<%=request.getContextPath()%>/product/reference?referenceInputType=radio','form.productId,form.productName');">选择</span>
 ...
 <!-- begin  详情 -->
	...
	<%--                      
		                       <a href="javascript:void(0)" title="审核通过" onclick="javascript:doAudit('audit');"  id="audit">
		                           
		                           <i class="icon-check" ></i>
		                         
		                         </a>

		                         <a href="javascript:void(0)" title="审核驳回"  onclick="javascript:doAudit('noAudit');"  id="noAudit" >
 ...
 --%>
			                          <tr>
			                             ...
			                             <td>
			                             	<c:out value="${price.AVAILABLECREDIT }"></c:out>
			                             </td>
			                          </tr>
			             ...
			             <%--    <tr>
		                   ...
		                   <div class="row">
							<div class="col-xs-12">
		                      ...
		                      <!-- begin 操作按钮 -->
		                         <div class="opt-div">
		                          <a href="javascript:void(0);" class="save-btn-shi"  onclick="javascript:doAudit('audit');"  id="audit">审核通过</a>
		                          <a href="javascript:void(0);" class="sub-btn-shi" onclick="javascript:doAudit('noAudit');"  id="noAudit"  >审核驳回</a>
