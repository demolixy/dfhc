				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li class="active">
								<c:choose>
									<c:when test="${completionConfirm == '1' }">装车完成确认</c:when>
									<c:otherwise>装车单打印</c:otherwise>
								</c:choose>
							</li>
						</ul><!-- .breadcrumb -->
						<%-- 
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/loadingnotice/ajaxList?${queryString}',
					datatype: "json",
				    mtype: 'POST',  
					height: '100%',
					colNames:[
...
,
			...
			function printing(){
                ...
                var pointer=$(".cbox:checked").parents("tr");
                var updateDate=$(pointer).attr("id");
                window.location = "${pageContext.request.contextPath}/loadingnotice/printladingbill?ladingBillId=" + updateDate + "&date=" + new Date();
			}
		</script>
	</body>
</html>
