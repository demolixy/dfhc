   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("ladingBillId")%></span><input type="text" class="text_field_reference text_field_readonly"  hiddenInputId="ladingBillId" name="ladingBillIdName" inputName="<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("ladingBillId")%>" value="" /><input type="hidden" name="ladingBillId"><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onclick="javascript:getReference(new Array(form.ladingBillId, form.ladingBillIdName), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/ladingbill/reference?referenceInputType=radio');"/>
							</div>
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("sequenceId")%></span><input class="m-wrap" name="sequenceId" id="sequenceId" type="text"  						maxLength="19" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/rownumber/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/rownumber/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: 250,
					colNames:['id',
'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("ladingBillId")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("rowType")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("rowDate")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("sequence")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("status")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("truckNo")%>'
,'<%=IRowNumberConstants.TABLE_COLUMN_DISPLAY.get("sequenceId")%>'
],
					colModel:[
			...
			{name:'rowType',index:'rowType', width:90,editable: true,editoptions:{size:"20",maxLength:"2"}},
			{name:'rowDate',index:'rowDate',width:90, editable:true, sorttype:"date",unformat: pickDate}
,
			{name:'sequence',index:'sequence', width:90,editable: true,editoptions:{size:"20",maxLength:"10"}},
			...
			{name:'sequenceId',index:'sequenceId', width:90,editable: true,editoptions:{size:"20",maxLength:"19"}}
					], 	
		                        shrinkToFit:false,
					viewrecords : true,
					rownumbers:true,
