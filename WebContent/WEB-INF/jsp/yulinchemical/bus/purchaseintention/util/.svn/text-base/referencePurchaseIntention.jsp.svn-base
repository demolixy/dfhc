<%--
   ...
   *
   * 文件名称：referencePurchaseIntention.jsp
   *
   * 功能描述：购买意向管理参照页面
   * 
   * 版本历史：
   ...
   <form name="form" id="form" class="FormGrid ui_inner_form" method="post" >
           ...
           <div class="control-group">
							...
							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("customerName")%></span><input class="m-wrap" name="customerName" id="customerName" type="text"  						maxLength="200" />
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("productName")%></span><input class="m-wrap" name="productName" id="productName" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("requirement")%></span><input class="m-wrap" name="requirement" id="requirement" type="text"  						maxLength="300" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("requirementDate")%></span><input name="requirementDate" id="requirementDate" class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("requirementDate")%>" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("estimatedAmount")%></span><input class="m-wrap" name="estimatedAmount" id="estimatedAmount" type="text"  						maxLength="19" />
							</div>
							
							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("noteTaker")%></span><input class="m-wrap" name="noteTaker" id="noteTaker" type="text"  						maxLength="200" />
							</div>

							<div class="input-prepend">
							  <span class="add-on"><%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("recordTime")%></span><input name="recordTime" id="recordTime" class="m-wrap m-ctrl-medium date-picker" readonly size="16" type="text" value="" inputName="<%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("recordTime")%>" />
							</div>
     ...
     <div class="EditTable operate_div" >
       <a href="javascript:void(0)" id="sData" class="fm-button ui-state-default ui-corner-all fm-button-icon-left" onclick="javascript:searchShi('<%=request.getContextPath()%>/purchaseintention/reference?referenceInputType=${referenceInputType}');">
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					...
					url: getRootPath()+'/purchaseintention/referenceAjax',
					datatype: "json",
				    mtype: 'POST',  
					height: 250,
					colNames:['id',
...
,'<%=IPurchaseIntentionConstants.TABLE_COLUMN_DISPLAY.get("recordTime")%>'
			...
			{name:'requirementDate',index:'requirementDate',width:90, editable:true, sorttype:"date",unformat: pickDate},
			...
			{name:'recordTime',index:'recordTime',width:90, editable:true, sorttype:"date",unformat: pickDate}
