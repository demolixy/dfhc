						...
						<ul class="breadcrumb">
							...
							<c:choose>
								<c:when test="${operation == 'coordination' }">
									...
									<li class="active">客户发货情况查看</li>
								</c:when>
								<c:otherwise>
									<li>
										<a href="#">装车业务</a>
									</li>
									<li class="active">作废提货单</li>
								</c:otherwise>
							</c:choose>
...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'billType',index:'billType', width:90,editable: true,editoptions:{size:"20",maxLength:"2"}, formatter:billTypeConvert},
			...
			{name:'invalidPerson',index:'invalidPerson', width:90,editable: true,editoptions:{size:"20",maxLength:"20"}},
			{name:'invalidTime',index:'invalidTime', width:90, editable:true, sorttype:"date", formatter:formatDateRow},
			{name:'invalidReason',index:'invalidReason', width:90,editable: true,editoptions:{size:"20",maxLength:"20"}},
...
// 				      onClickButton: addShow, position:"last" }) ;
				...
				function showDetail(){
                    ...
                    var pointer=$(".cbox:checked").parents("tr");
                    var updateDate=$(pointer).attr("id");
                    window.location = "${pageContext.request.contextPath}/ladingbill/ladingBillDetailForBillType?ladingBillId=" + updateDate + "&date=" + new Date();
				}
			});
