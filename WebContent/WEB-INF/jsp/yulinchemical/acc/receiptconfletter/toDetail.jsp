...
<!-- begin  详情 -->
						...
						<div class="row">
							...
							<div class="opt-div">
		                          ...
		                          <a href="javascript:void(0);" class="save-btn-shi" id="save" onclick="confirmPurchase('${bean.id}');">确认结算</a>
...
,'<%=ILadingBillConstants.TABLE_COLUMN_DISPLAY.get("remark")%>'
			...
			{name:'actualShippingAmount',index:'actualShippingAmount',editable: true,editoptions:{size:"20",maxLength:"14"}},
			...
			function returnStatus(obj, flag){
				var url;
				if(flag){
					url = "<%=request.getContextPath()%>/receiptconfletter/updateStatus?operation=${operation}&id=" + obj.toString() + "&flag=" + flag + "&date=" + new Date();
					if(!confirm("是否确认确认结算？")){
						return false;
					}
				}else{
					url = "<%=request.getContextPath()%>/receiptconfletter/updateStatus?operation=${operation}&id=" + obj.toString() + "&date=" + new Date();
					if(!confirm("是否确认退回？")){
						return false;
					}
				}
