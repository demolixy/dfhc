							...
							<li>
							 ...
							 <c:choose>
								<c:when test="${operation == 'coordination' }">
									...
									<li class="active">历史结算开票信息</li>
								</c:when>
								<c:otherwise>
									<li>
										<a href="#">副产品结算业务</a>
									</li>
									<li class="active">副产品开发票申请</li>
								</c:otherwise>
							</c:choose>	
						</ul><!-- .breadcrumb -->
...
,
			...
			{name:'applyTime',index:'applyTime',width:90, editable:true, sorttype :"date",formatter: formatDateRow}
,
...
// 				      position:"last" }) ;
				<c:choose>
					...
					//审核
					  jQuery(grid_selector) .navButtonAdd(pager_selector,
						{ caption:"",title:"开发票申请", buttonicon:"icon-group red",
						  onClickButton:updateShow , 
					      position:"last" }) ;
					//查看
					  jQuery(grid_selector) .navButtonAdd(pager_selector,
			     ...
			     function updateShow(){
                    ...
                    if(checkedLen>1){
                     ...
                     $.ajax({
                        type: "POST",
                        url: "<%=request.getContextPath()%>/receiptconfletter/doApplySettlement/" + updateDate+"/"+new Date(),
                        dataType: "json",
                        ...
                        success: function(data,status) {
                	   	   if(data.status=="0"){
                	   	       alert(data.message);
                	   	        ...
                	   	        alert(data.message);
//         	                    window.location.href="${pageContext.request.contextPath}/receiptconfletter/listApplySettlement?pcode=${pcode}";
...
//                      $("#portlet-config-edit").show();
			   ...
			   //查看详情
					function to_detail(){
	                    ...
	                    window.location.href="${pageContext.request.contextPath}/receiptconfletter/applySettlementDetail?id="+id+"&pcode=${pcode}";
