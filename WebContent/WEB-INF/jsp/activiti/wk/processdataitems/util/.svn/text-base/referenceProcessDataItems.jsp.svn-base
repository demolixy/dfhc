...
<script type="text/javascript">
   ...
   $(function(){
         ...
         var url="<%=request.getContextPath()%>/processdataitems/reference";
        sorting(url);
   });

    function simpleQuery_onClick(){  //简单的模糊查询
        form.action="<%=request.getContextPath()%>/processdataitems/reference";
        form.submit();
    }
...
<body class="page-header-fixed">
  ...
  <!-- begain查询 -->
<form action="<%=request.getContextPath()%>/processdataitems/reference" name="form"	method="post">
						...
						<div class="portlet-body no-more-tables dataTables_wrapper form-inline" style="overflow: auto;" >
							<table class="table table-striped table-bordered table-hover table-full-width dataTable" aria-describedby="sample_2_info">
								<thead>
									...
									<th  tabindex="0" class="sorting" RM_ORDER_STR="ID" role="columnheader" aria-controls="sample_2" style="width: 273px;" aria-label="Browser: activate to sort column ascending" aria-sort="descending" rowspan="1" colspan="1"><%=IProcessDataItemsConstants.TABLE_COLUMN_DISPLAY.get("id")%></th>
