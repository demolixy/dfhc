...
<script type="text/javascript">
   ...
   $(function(){
         ...
         var url="<%=request.getContextPath()%>/processdataitems/referenceProcessDefinition";
        sorting(url);
   });

    function simpleQuery_onClick(){  //简单的模糊查询
        form.action="<%=request.getContextPath()%>/processdataitems/referenceProcessDefinition";
        form.submit();
    }
    ...
    var objKeys =['displayName','keystring'];
</script>
</head>
<body class="page-header-fixed">
  <div class="page-content"  style="margin: 0px 0px 0px 0px;padding: 0px 0px;">
  <!-- begain查询 -->
<form action="<%=request.getContextPath()%>/processdataitems/referenceProcessDefinition" name="form"	method="post">
	   ...
	   <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
			<div class="span12">
			  <ul class="breadcrumb">
					...
					<li><a href="#"><%=IProcessDataItemsConstants.TABLE_NAME_DISPLAY%>参照</a></li>
				</ul>
			</div>
	 </div>
    <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
						...
						<div class="portlet-body no-more-tables dataTables_wrapper form-inline" style="overflow: auto;" >
							<table class="table table-striped table-bordered table-hover table-full-width dataTable" aria-describedby="sample_2_info">
								...
								<td>
									<div class="checkbox" >
										<ins id="ischeck">
											<input type="hidden" name="checkbox_template" class="checkmain" ischecked="false"  displayName="${list.name}" keystring="${list.key}"  checkedId="<c:out value="${list.id}"></c:out>" />
										</ins>
									</div>
								</td>
						          <td><c:out value="${list.id}"></c:out></td>
						          <td><c:out value="${list.deploymentId}"></c:out></td>
						          <td><c:out value="${list.name}"></c:out></td>
						          <td><c:out value="${list.key}"></c:out></td>
