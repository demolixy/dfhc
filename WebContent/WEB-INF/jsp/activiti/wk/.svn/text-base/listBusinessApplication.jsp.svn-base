...
<form action="<%=request.getContextPath()%>/workflow/model/businessApplication"	method="post" name="form" >
								...
								<tbody>
									<c:forEach items="${beans}" var="list" varStatus="rowCounter"	begin='0' step='1'>
										<tr>
											...
											<td>${list.id }</td>
											<td>${list.name }</td>
											<td><a href="${ctx }/businessAppController/detail/${list.id}">业务申请</a></td>
										</tr>
								 </c:forEach>
								</tbody>
							</table>
 ...
 <script type="text/javascript">
	...
	$(function(){
         ...
         var url="<%=request.getContextPath()%>/workflow/model/businessApplication";
        sorting(url);
        });
	</script>		
</body>
