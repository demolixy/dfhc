...
<%
    String referenceInputType = String.valueOf(request.getAttribute(IDataSetConstants.REQUEST_REFERENCE_INPUT_TYPE));
    ...
    //取出List
    List<DataSetVo> lResult = null;  //定义结果列表的List变量
    if(request.getAttribute(IDataSetConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
        lResult = (List)request.getAttribute(IDataSetConstants.REQUEST_BEANS);  //赋值给resultList
    }
%>
...
<script type="text/javascript">
   ...
   $(function(){
         ...
         var url="<%=request.getContextPath()%>/dataset/reference";
        sorting(url);
   });

    function simpleQuery_onClick(){  //简单的模糊查询
        form.action="<%=request.getContextPath()%>/dataset/reference";
        form.submit();
    }
...
<body class="page-header-fixed">
  ...
  <!-- begain查询 -->
<form action="<%=request.getContextPath()%>/dataset/reference" name="form"	method="post">
	   ...
	   <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
			<div class="span12">
			  <ul class="breadcrumb">
					...
					<li><a href="#"><%=IDataSetConstants.TABLE_NAME_DISPLAY%>参照</a></li>
				</ul>
			</div>
	 </div>
    <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
						...
						<div class="portlet-body no-more-tables dataTables_wrapper form-inline" style="overflow: auto;" >
							<table class="table table-striped table-bordered table-hover table-full-width dataTable" aria-describedby="sample_2_info">
								<thead>
									...
									<th  tabindex="0" class="sorting" RM_ORDER_STR="ID" role="columnheader" aria-controls="sample_2" style="width: 273px;" aria-label="Browser: activate to sort column ascending" aria-sort="descending" rowspan="1" colspan="1"><%=IDataSetConstants.TABLE_COLUMN_DISPLAY.get("id")%></th>
								...
								<td>
						          ...
						          <td><c:out value="${list.dataSetType}"></c:out></td>
