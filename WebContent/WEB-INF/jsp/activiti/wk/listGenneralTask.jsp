...
<form action="<%=request.getContextPath()%>/generalbusinessprocess/taskList"	method="post" name="form" >
								...
								<tbody>
									<c:forEach items="${beans}" var="list" varStatus="rowCounter"	begin='0' step='1'>
										<tr>
											...
											<td>
												<c:if test="${empty list.assignee }">
													<a class="btn" href="claim/${list.id}"><i class="icon-eye-open"></i>签收</a>
												</c:if>
												<c:if test="${not empty list.assignee }">
													<a class="btn" href="view/${list.id}"><i class="icon-user"></i>办理</a>
												</c:if>
											</td>
										</tr>
								 </c:forEach>
 ...
 <script type="text/javascript">
	...
	$(function(){
         ...
         var url="<%=request.getContextPath()%>/generalbusinessprocess/taskList";
        sorting(url);
        });
	</script>		
</body>
