...
<%@page import="com.dfhc.rm.user.vo.UserVo" %>
<%@page import="com.dfhc.rm.user.IUserConstants" %>
<%
    String referenceInputType = String.valueOf(request.getAttribute(IUserConstants.REQUEST_REFERENCE_INPUT_TYPE));
    ...
    //取出List
    List<UserVo> lResult = null;  //定义结果列表的List变量
    if(request.getAttribute(IUserConstants.REQUEST_BEANS) != null) {  //如果request中的beans不为空
        lResult = (List)request.getAttribute(IUserConstants.REQUEST_BEANS);  //赋值给resultList
    }
%>
...
<script type="text/javascript">
   ...
   $(function(){
         ...
         var url="<%=request.getContextPath()%>/user/reference";
        sorting(url);
   });

    function simpleQuery_onClick(){  //简单的模糊查询
        form.action="<%=request.getContextPath()%>/user/reference";
        form.submit();
    }
...
<body class="page-header-fixed">
  ...
  <!-- begain查询 -->
<form action="<%=request.getContextPath()%>/user/reference" name="form"	method="post">
	   ...
	   <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
			<div class="span12">
			  <ul class="breadcrumb">
					...
					<li><a href="#"><%=IUserConstants.TABLE_NAME_DISPLAY%>参照</a></li>
				</ul>
			</div>
	 </div>
    <div class="row-fluid" style="margin: 0 0px;padding: 0px 0px;">
						...
						<div class="portlet-body no-more-tables dataTables_wrapper form-inline" style="overflow: auto;" >
							<table class="table table-striped table-bordered table-hover table-full-width dataTable" aria-describedby="sample_2_info">
								<thead>
									...
									<th class="sorting" RM_ORDER_STR="LOGIN_ID"><%=IUserConstants.TABLE_COLUMN_DISPLAY.get("loginId")%></th>
									...
									<th class="sorting" RM_ORDER_STR="EMPLOYEE_ID"><%=IUserConstants.TABLE_COLUMN_DISPLAY.get("employeeId")%></th>
								...
								<td>
						          ...
						          <td><c:out value="${list.employeeId}"></c:out></td>
