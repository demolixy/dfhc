	    ...
	    <td>
			<table width=100% align=center border=0 bgcolor="#ffffff">
				<tr><input type="hidden" name="old_code_id" value="<%=RmStringHelper.prt(old_code_id)%>"/>
					<td align=center>
						<iframe id="iframe_role_function_node_tree" name="iframe_role_function_node_tree" frameborder="0" width="100%" height="400" src="<%=request.getContextPath()%>/orgauth/zTree/functionNodeOrgZtree.jsp?submitType=submitAll&nodeRelationType=hasRelation&lazy_init=0&inputType=checkbox&enableCookie=false"></iframe>
