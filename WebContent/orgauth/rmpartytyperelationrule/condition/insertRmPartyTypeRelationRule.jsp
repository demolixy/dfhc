...
<%  //判断是否为修改页面
	...
	if(request.getParameter("isModify") != null) {  //如果从request获得参数"isModify"不为空
	 ...
	 String party_view_id=request.getParameter("party_view_id")==null?"":request.getParameter("party_view_id");
     String party_view_id_name=request.getParameter("party_view_id_name")==null?"":request.getParameter("party_view_id_name");
%>
...
<table class="mainTable">
	...
	<tr>
		...
		<td>
			<input type="text" class="text_field_reference" validate='notNull;' hiddenInputId="party_view_id" name="party_view_id_name" inputName="<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("party_view_id")%>" value="<%=party_view_id_name %>" />
			<input type="hidden" name="party_view_id" value="<%=party_view_id %>"></input>
			<img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onClick="javascript:getReferenceOld(new Array(form.party_view_id, form.party_view_id_name), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/RmPartyViewAction.do?cmd=queryReference&referenceInputType=radio');"/>
		</td>
		<td align="right"></td>
		<td></td>
	</tr>
	<tr>
		...
		<td>
			<input type="text" class="text_field_reference"  hiddenInputId="parent_party_type_id" name="parent_party_type_id_name" inputName="<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("parent_party_type_id")%>" value="" />
			<input type="hidden" name="parent_party_type_id"></input>
			<img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onClick="javascript:getReferenceOld(new Array(form.parent_party_type_id, form.parent_party_type_id_name), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/RmPartyTypeAction.do?cmd=queryReference&referenceInputType=radio');"/>
		</td>
		<td align="right"></td>
		<td></td>
	</tr>
	<tr>
		...
		<td>
			<input type="text" class="text_field_reference" validate='notNull;' hiddenInputId="child_party_type_id" name="child_party_type_id_name" inputName="<%=IRmPartyTypeRelationRuleConstants.TABLE_COLUMN_CHINESE.get("child_party_type_id")%>" value="" />
			<input type="hidden" name="child_party_type_id"></input><img class="refButtonClass" src="<%=request.getContextPath()%>/images/09.gif" onClick="javascript:getReferenceOld(new Array(form.child_party_type_id, form.child_party_type_id_name), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/RmPartyTypeAction.do?cmd=queryReference&referenceInputType=radio');"/>
		</td>
		<td align="right"></td>
		<td></td>
	</tr>
