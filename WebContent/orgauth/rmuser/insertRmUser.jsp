...
<script type="text/javascript" src="<%=request.getContextPath()%>/js/app-pub.js"></script>
...
<script type="text/javascript">
	...
	function openTree(organization_id,organization_name){ //组织树
		...
		var urlPath = '<%=request.getContextPath()%>/orgauth/tree/org.jsp?cmd=<%=IOrgauthConstants.OrgTree.DEFAULT.value()%>&enableCookie=true' + 
			"&defaultSelectedNodesValue=" + organization_id + 
			'&inputType=radio&parent_party_id=<%=parent_party_id %>&submit_bk=<%=request.getAttribute(IRmUserConstants.SUBMIT_BK)%>&include_self=1&view_id=' + sViewId;			
...
<%if(IOrgauthConstants.Config.isUserRelationParty.value()) { %>
	<tr>
		<td align="right">所属组织：</td>
	  	...
	  	<input type="text" class="text_field_reference"  name="organization_name" inputName="名称" <%if(isModify){ %> value="<%=resultVo.getAttribute1()%>" <%} %>/>
	  	<input type="hidden" class="organization_id" name="organization_id" <%if(isModify){ %>value="<%=resultVo.getOrganization_id() %>" <%} %>></input>
<!--	  	<img class="refButtonClass " src="<%=request.getContextPath() %>/images/09.gif" onClick="javascript:getPartyWindow(new Array(form.organization_id, form.organization_name), '<%=request.getContextPath()%>/', '<%=request.getContextPath()%>/orgauth/tree/org.jsp?cmd=department&enableCookie=true&inputType=radio&show_bk=<%=request.getAttribute(IRmUserConstants.SUBMIT_BK)%>')"/>-->
	  	<img class="refButtonClass " src="<%=request.getContextPath() %>/images/09.gif" onClick="javascript:getHangingUserParty('<%=request.getContextPath()%>/pubController/getParty','organization_id,organization_name','form')"/>
...
<%} %>
	...
	<input type="hidden" name="oldName" value="<%= resultVo!=null ? resultVo.getName():""%>"></input>
	...
	<% } %>
    <input name="user_type" type="hidden"  value="<%=ISystemConstant.DICTIONARY_USER_TYPE_0 %>"  />
	<input type="hidden"  name="user_id" value="<%=resultVo != null ?resultVo.getId():""%>" id="user_id"></input>
</form>
</body>
</html>
<script type="text/javascript">
